...
def Figures_to_HTML(file):
    ...
    for figure_env in figure_envs:
        ...
        if include_graphics_match:
            ...
            replacement = f'<br><figure><img src="Visuals/svg/{filename}" style="width:100%; height:auto;"><figcaption>{caption_match.group("caption")}</figcaption> </figure>'
...
def latex_to_html_headings(latex_content):
    ...
    latex_content = re.sub(r'\\chapter\{(.+?)\}', lambda m: '</div> \n <div id="' + m.group(1).replace(' ', '').replace(':', '') + '_div" class="chapter"> \n <h1 id="' + m.group(1).replace(' ', '').replace(':', '') + '">' + m.group(1) + '</h1>', latex_content)
...
def JS_input(file_path):
    ...
    with open(file_path, 'w') as file:
        for line in lines:
            ...
            if match:
                # Replace the line with the div and jQuery script
                content = match.group(1)
                line = f'<div id="Diagrams{content}" class="Animated_Diagram">\n'
                line += f'<script> $("#Diagram{content}").load("Diagram/{content}.html"); </script>\n'
                line += '</div>\n'
            file.write(line)
...
create_html_with_mathjax(Latex_File, '../Special_Relativity.html')

# Call the function with your HTML file
create_toc('../Special_Relativity.html')