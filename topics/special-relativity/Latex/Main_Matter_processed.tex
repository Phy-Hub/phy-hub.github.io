	...
	\begin{subfigure}{0.45\textwidth}
		...
		\begin{tikzpicture}
			...
			\foreach \a in {0, 15, ..., 345} {
					...
					\coordinate (H) at (\a:1);
					%\draw[-{Latex[length=1.5mm, width=1.5mm]}, thick]
					\draw[-stealth, thick] ($(\TTH ) + 0.5*(\cosb, \sinb)$) -- (H);
				}
		\end{tikzpicture}
...
We will come across a very useful quantity which we will denote with a ${\TTgamma }$, this quantity will be used throughout this book to simplify equations.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[scale = 3]%,tdplot_main_coords]
		...
		%
		\draw[black, thick,->] (1.5,0,0)--(2.1,0,0) node[anchor = north east]{${\TTx }$};
		\draw[black, thick,->] (1.7,-0.2,0)--(1.7,0.45,0) node[anchor = west]{${\TTz }$};
		\draw[black, thick,->] (1.7,0,0.3)--(1.7,0,-0.7) node[anchor = south]{${\TTy }$};
		%
		\draw[gray, dashed, thick,->] (1.5,0.7,0)--(2.1,0.7,0) node[anchor = north east]{${\TTxPrime  }$};
		\draw[gray, dashed, thick,->] (1.7,0.5,0)--(1.7,1.2,0) node[anchor = west]{${\TTzPrime  }$};
		\draw[gray, dashed, thick,->] (1.7,0.7,0.3)--(1.7,0.7,-0.8) node[anchor = south]{${\TTyPrime  }$};
		\draw[gray, thick,->>] (1.55,0.7,0)--(1.55,0.92,0) node[anchor = east]{${\TTv }$};
		\fill[red] (3.03,0.9,0) circle (0.6pt);
		%
		\draw[red, thick,->] (1.7,0,0)--(3.02,0.87,0) node[midway,anchor = north]{${\TTunderliner  }$};
		\draw[red, thick,->] (1.7,0,0)--(1.7,0.7,0) node[midway,anchor = east]{${\TTvecOOPrime }$};
		\draw[red, thick,->] (1.7,0.7,0)--(3,0.9,0) node[midway,anchor = south]{${\TTvecOPrimeE  }$};
		%
	\end{tikzpicture}
	\caption{\textbf{An event in initial frame with a primed frame's axis moving relative to its axis.}	This shows an initial frame with its associated $\TTx ,\TTy ,\TTz $-coordinate system at rest in this frame, with origin ${\TTO }$. There is also an $\TTxPrime ,\TTyPrime ,\TTzPrime $-coordinate system with origin ${\TTOPrime }$ associated with the "primed" frame, at position ${\TTvecOOPrime  }=(0,0,{\TTv }{\TTt })$ in the initial frame, where ${\TTv }$ is the speed of the primed frame in the z-direction relative to the initial frame, and ${\TTt }$ is the time in the initial frames. An event ${\TTvecE  }$ in this initial frame is described by a set of spatial and time coordinates ${\TTunderliner  } = ({\TTx },{\TTy },{\TTz })$ and ${\TTt }$. We choose the time to be zero in both frames when both axes overlapped, so that ${\TTt }$ is also the time from when they overlapped. We also show ${\TTvecOPrimeE  }$ which represents the vector from the primed axis origin to the event in the initial frame.}
	\label{fig: simple Galilean transform derivation}
\end{figure}
%███████████

As shown in the diagram, we are in the initial frame with origin ${\TTO }$ defined by coordinates $(0,0,0)$ and the primed frame is moving in the z-direction at velocity ${\TTv }$ such that the position of the primed origin relative to the initial frame's axis is

\begin{equation}
	\TTvecOOPrime  = (0,0,{\TTv }{\TTt })
\end{equation}

at time ${\TTt }$ from when the origins overlapped.
If there is an \hyperlink{def-event}{event} at position

\begin{equation}
	\TTunderliner  = ({\TTx },{\TTy },{\TTz })
\end{equation}

the event is described by

\begin{equation}
	\TTvecE  = ({\TTx },{\TTy },{\TTz },{\TTt })
\end{equation}

then in the primed frame, we will have the event described by the primed coordinates that we want to transform into, as

\begin{equation}
	\TTvecEPrime  = ({\TTxPrime  },{\TTyPrime  },{\TTzPrime  },\TTtPrime )
\end{equation}

To swap into the primed frame's coordinates, we need to recall from the previous assumptions that distances and times are conserved, and therefore the length and directions of all the vectors between ${\TTO }$, ${\TTOPrime }$ and ${\TTE }$ are the same in each frame, and time flows at the same rate (${\TTtPrime } = {\TTt }$), from this knowledge and Figure (\ref{fig: simple Galilean transform derivation}) we have the event's coordinates described in the primed frame by its coordinate axis as

\begin{equation}
	{\TTunderlinerPrime  } = ({\TTxPrime  },{\TTyPrime  },{\TTzPrime  }) = \TTunderliner -\TTvecOOPrime  = ({\TTx },{\TTy },{\TTz }-{\TTv }{\TTt })
...
\begin{equation}
	\mhl{
		\begin{aligned}
			 & {\TTxPrime  } = {\TTx }        \\
			 & {\TTyPrime  } = {\TTy }        \\
			 & {\TTzPrime  } = {\TTz }-{\TTv }{\TTt } \\
			\text{at time: }\quad&  {\TTtPrime  }= {\TTt }
		\end{aligned}
	}
...
\begin{equation}
	\begin{aligned}
		 & {\TTx } = {\TTxPrime  }             \\
		 & {\TTy } = {\TTyPrime  }             \\
		 & {\TTz } = {\TTzPrime  } + {\TTv }{\TTtPrime  } \\
		\text{at time: }\quad&  {\TTt } = {\TTtPrime  }
	\end{aligned}
...
This can be seen at time ${\TTt } = 0$ when we have the frame's origins overlapping and there is a symmetry from here, as we have the freedom to choose either frame to be the original, as no frame is particularly special and depending on which one you pick the relative velocity is either ${\TTv }$ or-${\TTv }$. We will have the same rules in special relativity later on when we want to get the inverse transform.
...
\begin{figure}[H]
	\begin{subfigure}{.49\textwidth}
		...
		\begin{tikzpicture}[scale = 0.65]
			...
			\draw[red, dashed, thick arrows] (6, 6.15)--(0, 6.15) node[midway, above] {$\frac{1}{2}{\TTc }{\TTDelta  \TTt }$};

			% Draw the distance labels
			\draw[<->, thick] (0, 4.7)--(6, 4.7) node[midway, above] {${\TTL }$};
	...
	\begin{subfigure}{.49\textwidth}
		...
		\begin{tikzpicture}[scale = 0.65]
			...
			% Draw the dashed arrowed lines with custom thick arrowheads
			\draw[red, dashed, thick arrows] (A3)--($(\TTB 2)!0.02!(\TTA 3)$) node[midway, below] {$\frac{1}{2}{\TTc }{\TTDelta  \TTtPrime  }$};
			\draw[red, dashed, thick arrows] (B2)--(A1) node[midway, below] {$\frac{1}{2}{\TTc }{\TTDelta  \TTtPrime  }$};

			% Draw the velocity arrow
			\draw[black, thick,->, line width = 1.5pt] (3, 6)-- +  + (0, 0.5) node[midway, right] {${\TTu }$};

			% Draw the distance labels
			\draw[<->, thick] (0,-1.3)--(6,-1.3) node[midway, above] {${\TTL }$};
			\draw[->, thick] (-1, 0)--(-1, 6) node[midway, left] {${\TTu }{\TTDelta  \TTtPrime  }$};

		\end{tikzpicture}
		\caption{Moving frame}
	\end{subfigure}
	\caption{\textbf{Light clock showing time dilation.} The light clock keeps time by counting each time the light makes a return journey when bouncing between two mirrors with length ${\TTL }=\frac{1}{2}{\TTc }{\TTDelta  \TTtau }$ between them, were ${\TTDelta \TTtau }$ is the time for the return journey in the rest frame, the diagram shows the mirror's rest frame (left) and a frame with them moving at speed ${\TTu }=-{\TTv }$ (right). In this second frame, there is a distance ${\TTu }{\TTDelta  \TTtPrime  }$ between where the light was initially reflected by the left mirror and where it returned to it. For this, the light had to travel a total diagonal distance of ${\TTc }{\TTDelta  \TTtPrime  }$ to return, were ${\TTDelta \TTtau }$ is the time for the return journey in the primed frame.}
	\label{fig: light clock}
\end{figure}
%███████████

The light in the clock, takes a time ${\TTDelta \TTtau }$ in the clock's rest frame to make the return journey.
...
\begin{equation}
	2{\TTL } = {\TTc }{\TTDelta \TTtau }.
\end{equation}

The distance the mirrors move during this return journey in the moving frame is

\begin{equation}
	{\TTu }{\TTDelta  \TTtPrime  } =-{\TTv } {\TTDelta  \TTtPrime  },
\end{equation}

where ${\TTDelta  \TTtPrime  }$ is the time it took light to return in the moving frame and ${\TTu }$ is the speed of the light clock, which is in the opposite direction of the movement of the \hyperlink{def-Primed-Frame}{primed frame} relative to the \hyperlink{def-proper-frame}{rest frame}, that is ${\TTu } =-{\TTv }$.

The distance light travels in this time is

\begin{equation}
	{\TTc }{\TTDelta  \TTtPrime  }
\end{equation}

This is the hypotenuse of a right-angled triangle formed by ${\TTu }{\TTDelta  \TTtPrime  }=-{\TTv }{\TTDelta  \TTtPrime  }$ and $2{\TTL }$, so we have the square of the hypotenuse equal to the square of the other two lengths:

\begin{equation}
	{\TTc }^2 {\TTDelta  \TTtPrime  }^2 = {\TTv }^2 {\TTDelta  \TTtPrime  }^2 + {\TTc }^2{\TTDelta \TTtau }^2
\end{equation}
...
\begin{equation}
	...
	{\TTDelta  \TTtPrime  } = {\TTgamma } {\TTDelta \TTtau }
\end{equation}

with

\begin{equation}
	{\TTgamma } = \frac{1}{\sqrt{1-\frac{ {\TTv }^2}{ {\TTc }^2} } }.
\end{equation}

This factor, ${\TTgamma }$, is used throughout special relativity to simplify equations.
...
\begin{equation}
	{\TTDelta  \TTtPrime  } \geq {\TTDelta \TTtau }
\end{equation}
...
\begin{figure}[H]
	\begin{subfigure}{.49\textwidth}
		...
		\begin{tikzpicture}[scale = 0.55]
			...
			\draw[red, dashed, thick arrows] (-0.15, 0)--(-0.15, 6) node[midway, left] {${\TTzOne }$};
			\draw[red, dashed, thick arrows] (0.15, 6)--(0.15, 0) node[midway, right] {${\TTzTwo }$};

			% Draw the distance labels
			\draw[<->, thick] (-1.5, 0)--(-1.5, 6) node[midway, left] {${\TTDelta  \TTZ }$};
	...
	\begin{subfigure}{.49\textwidth}
		...
		\begin{tikzpicture}[scale = 0.55]
			...
			% Draw the dashed arrowed lines with custom thick arrowheads
			\draw[red, dashed, thick arrows] ($(\TTP 1)+(-0.15,0)$)--($(\TTP 3)+(-0.15,0)$) node[midway, left] {${\TTzPrimeOne }$};
			\draw[red, dashed, thick arrows] ($(\TTP 3)+(0.15,0)$)--($(\TTA 1)+(0.15,0)$) node[midway, right] {${\TTzPrimeTwo }$};
			\draw[red, dashed, thick arrows] (P1)--(P2);
			\draw[red, dashed, thick arrows] (P2)--(A1);

			\draw[<->, thick] ($(\TTA 1)+(-1.5,0)$)--($(\TTC 1)+(-1.5,0)$) node[midway, left] {${\TTDelta  \TTZPrime }$};

			\draw[black, thick arrows, line width = 1.5pt] (3, 4)-- +  + (0, 0.9) node[midway, right] {${\TTu }$};
		\end{tikzpicture}
		\caption{Moving frame}
	\end{subfigure}
	\caption{\textbf{Light paths showing the need for length contraction.} A central orange bulb emits light in two directions, with all light being reflected by the mirrors back to reach the central bulb simultaneously. This is shown at rest, fig (a), and in another inertial frame with the system now moving at speed ${\TTu }$, fig (b), with its length now having to be contracted in the direction of movement to allow for the simultaneous return of light to the bulb. The length between the bulb and front mirror is shown as ${\TTDelta  \TTZ }$ and ${\TTDelta  \TTZPrime }$ in each frame. The paths the light took to the front mirror ${\TTzOne }$ and back ${\TTzTwo }$ are shown in the rest frame with the corresponding paths ${\TTzPrimeOne }$ and ${\TTzPrimeTwo }$ in the moving frame.}
...
In the moving frame, to achieve this simultaneity in the return of the light to the bulb, the length of the full path of light from the central bulb to the front mirror and back ($2 {\TTDelta  \TTZPrime } = {\TTzPrimeOne } + {\TTzPrimeTwo }$) must be the same as the perpendicular path, due to the speed of light being the same for both paths.
We can work out the length of the sideways paths from the time dilation section, ${\TTc }{\TTDelta  \TTtPrime }$, and this is the length the total path needs to be in the forward directions as well.
...
\begin{equation}
	{\TTzPrimeOne } = {\TTc }{\TTDelta  \TTtPrimeOne } = {\TTDelta   \TTZPrime } + {\TTu } {\TTDelta  \TTtPrimeOne }
\end{equation}

where ${\TTu }$ is the speed that the front mirror is moving away from the point it was emitted, ${\TTDelta  \TTtPrimeOne }$ is the time taken to get to the mirror, and ${\TTDelta   \TTZPrime }$ is the distance between the bulb and the mirror in front in the primed frame.
After reaching the mirror, the light then must travel back to the bulb a distance

\begin{equation}
	{\TTzPrimeTwo } = {\TTc } {\TTDelta  \TTtPrimeTwo } = {\TTDelta   \TTZPrime } - {\TTu } {\TTDelta  \TTtPrimeTwo }.
\end{equation}

Where ${\TTDelta  \TTtPrimeTwo }$ is the time taken to return to the bulb from the front mirror, with the source moving towards the reflected light.

Both of these give the two times as

\begin{equation}
	{\TTDelta  \TTtPrimeOne } = \frac{\TTDelta   \TTZPrime }{ {\TTc } - {\TTu } }
\end{equation}

and

\begin{equation}
	{\TTDelta  \TTtPrimeTwo } = \frac{ {\TTDelta   \TTZPrime } }{ {\TTc } + {\TTu } }.
\end{equation}
...
\begin{equation}
	\begin{aligned}
		{\TTDelta  \TTtPrime  } & \color{gray} = {\TTDelta   \TTZPrime }(\frac{1}{ {\TTc } + {\TTu } } + \frac{1}{ {\TTc } - {\TTu } }) \\
		              & \color{gray} = \frac{\TTDelta   \TTZPrime }{\TTc }\frac{2}{1- {\TTu }^2/{\TTc }^2}            \\
		              & =  2{\TTgamma }^2 \frac{\TTDelta   \TTZPrime }{\TTc }
	\end{aligned}
\end{equation}

where we used ${\TTu }=-{\TTv }$ and then the ${\TTgamma }$ factor to simplfy equation.
Now from this and using the time dilation equation, we have

\begin{equation}
	{\TTc }{\TTDelta  \TTtPrime  } = {\TTc }{\TTgamma } {\TTDelta  \TTt } = 2{\TTgamma } {\TTDelta  \TTZ } = 2{\TTgamma }^2 {\TTDelta   \TTZPrime }
\end{equation}

Finally, this leads to the length from the bulb to the front mirror in the primed frame to be

\begin{equation}
	{\TTDelta   \TTZPrime } = \frac{1}{ {\TTgamma } }{\TTDelta   \TTZ }.
\end{equation}
...
\begin{equation}
	{\TTDelta   \TTZPrime } \leq {\TTDelta   \TTZ }.
\end{equation}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[scale = 3]%,tdplot_main_coords]
		...
		%
	\end{tikzpicture}
	\caption{\textbf{An event in initial frame with a primed frame's axis moving relative to it.} This shows an initial frame with its associated $\TTx ,\TTy ,\TTz $-coordinate system at rest in this frame, with origin ${\TTO }$. There is also an $\TTxPrime ,\TTyPrime ,\TTzPrime $-coordinate system with origin ${\TTOPrime }$ associated with the "primed" frame, at position ${\TTvecOOPrime  }=(0,0,{\TTv }{\TTt })$ in the initial frame, where ${\TTv }$ is the speed of the primed frame in the z-direction relative to the initial frame, and ${\TTt }$ is the time in the initial frames. An event ${\TTvecE  }$ in this initial frame is described by a set of spatial and time coordinates ${\TTunderliner  } = ({\TTx },{\TTy },{\TTz })$ and ${\TTt }$. We choose the time to be zero in both frames when both axes overlapped, so that ${\TTt }$ is also the time from when they overlapped. We also show ${\TTvecOPrimeE  }$ which represents the vector from the primed origin to the event in the initial frame.}
	\label{fig: simple Lorentz transform derivation}
\end{figure}
%███████████

As shown in the diagram, we are in the initial frame with origin ${\TTO }$ defined by coordinates $(0,0,0)$ and the primed frame is moving in the z-direction at velocity ${\TTv }$ such that the position of the primed origin in the initial frame is
...
\begin{equation}
	\TTvecOPrimeE  = ({\TTx },{\TTy },{\TTz }-{\TTv }{\TTt })
\end{equation}
...
\begin{equation}
	{\TTunderlinerPrime  } = ({\TTxPrime  },{\TTyPrime  },{\TTzPrime  })
\end{equation}

These are just the position coordinates of the event in the primed frame.

The primed frame's axis and coordinates are moving relative to the initial frame, so from the previous length contraction section we would have the displacement between the primed axis origin and the event in the initial frame $\TTvecOPrimeE $, length contracted, in the $\TTz $-direction, compared to the primed position coordinates in the primed frame ${\TTunderlinerPrime  }$, that is

\begin{equation}
	\TTvecOPrimeE  = ({\TTx },{\TTy },{\TTz }-{\TTv }{\TTt }) = ({\TTxPrime  },{\TTyPrime  },\frac{\TTzPrime  }{\TTgamma })
\end{equation}

The $\TTx $ and $\TTy $-components are unaffected, with the paradox of it they were effected, explained in Figure (\ref{fig: width contraction}).
Rearranging for the $\TTz $-component, we have

\begin{equation}
	({\TTxPrime  },{\TTyPrime  },{\TTzPrime  }) = {\TTunderlinerPrime  } = ({\TTx },{\TTy },{\TTgamma }({\TTz }-{\TTv }{\TTt }) )
\end{equation}

There is a symmetry between the frames because if we were originally in the primed frame we would have the initial frame moving instead at -${\TTv }$ in the z-direction relative to the primed frame, and we could follow the above steps starting with this and the primed coordinate instead, and find the inverse \hyperlink{def-transform}{transformation} is the same apart from it replaces each frame's coordinates with the other and has the negative of the previous frame velocity instead.
That is

\begin{equation}
	({\TTx },{\TTy },{\TTz }) = {\TTunderliner  } = ({\TTxPrime  },{\TTyPrime  },{\TTgamma }({\TTzPrime  }+{\TTv }{\TTtPrime  }) )
\end{equation}

We can substitute both $\TTz $-coordinate transforms into each other to find the time transform

\begin{equation}
	{\TTz } = {\TTgamma } ( {\TTgamma } ({\TTz }-{\TTv }{\TTt }) + {\TTv }{\TTtPrime  })
\end{equation}

and rearranging gives

\begin{equation}
	{\TTtPrime  } = {\TTgamma } \left( \left( \dfrac{1}{ {\TTgamma }^2}-1 \right)\frac{\TTz }{\TTv } + \TTt  \right)
\end{equation}

with $1/{\TTgamma }^2 = 1-{\TTv }^2/{\TTc }^2$ leading to the primed frames time for the event given in initial frames coordinates as

\begin{equation}
	{\TTtPrime  } = {\TTgamma } \left( \TTt -\dfrac{ {\TTv }{\TTz } }{ {\TTc }^2} \right)
\end{equation}
...
\begin{equation}
	{\TTt } = {\TTgamma } \left( \TTtPrime +\dfrac{ {\TTv }{\TTzPrime  } }{ {\TTc }^2} \right)
\end{equation}
...
\begin{equation}
	\mhl{
		\begin{aligned}
			 & {\TTxPrime  } = \TTx                                            \\
			 & {\TTyPrime  } = \TTy                                            \\
			 & {\TTzPrime  } = {\TTgamma } ({\TTz }-{\TTv }{\TTt })                       \\
			\text{with the time since the}&\text{ overlap of the coordinate axes:} \\
			& {\TTtPrime  } = {\TTgamma } \bigg(\TTt -\frac{ {\TTv }{\TTz } }{ {\TTc }^2}\bigg) \\
			\text{with }\quad&  {\TTgamma } = \dfrac{1}{\sqrt{1-\frac{ {\TTv }^2}{ {\TTc }^2} } }
		\end{aligned}
	}
...
In the time \hyperlink{def-transform}{transform} equation the $\frac{ {\TTv }{\TTz } }{ {\TTc }^2}$ term is the cause of the \hyperlink{def-simultaneity}{simultaneity} of \hyperlink{def-event}{event}s at two different positions in one inertial frame no longer being \hyperlink{def-simultaneity}{simultaneous} in another inertial frame, and the ${\TTgamma }$ factor, is the term that causes the overall change in how fast time flows at a point relative to the initial frame.
In the spatial coordinate transform equations the ${\TTv }{\TTt }$ factor is due to both axes' origins moving away from each other and the ${\TTgamma }$ factor is the cause of the change of lengths in the direction of the frame's movement.
...
\begin{figure}[H]
	...
	\begin{subfigure}{0.29\textwidth}
		...
		\caption{primed frame (${\TTtPrime  }=0$)}
		\label{fig: coordinate transform subfig_3}
	\end{subfigure}
	\caption{\textbf{Transformation of coordinates into a primed frame.} A diagram, showing a grid of points, representing the initial frame's coordinates, at time ${\TTt } = 0$ that are at rest in their frame (left), giving the direct transformation, using the transform equations, into the moving primed frame (middle) where the points are now moving in the negative z-direction with ${\TTzPrime  } = {\TTgamma } {\TTz }$, the colors and labels show the desynchronization of the primed times ${\TTtPrime  } = -\frac{ {\TTv } }{ {\TTc }^2} {\TTgamma } {\TTz }$, the next figure (right) shows where these points would be when they are synchronized with the primed origin's time ${\TTtPrime  } = 0$, which can be worked out to be (${\TTzPrime  } = \frac{ {\TTz } }{ {\TTgamma } }$).}
	\label{fig: coordinate transform}
\end{figure}
%███████████

This diagram shows stationary points representing coordinates in the initial frame.
In the primed frame, these points are transformed to the position ${\TTzPrime  } = {\TTgamma } {\TTz }$ and are now moving at speed $-{\TTv }$, they are transformed using the transformation equations to points that are more spread out in the primed frame, but the times they are located at each of these points are different, they have desynchronized times.
Therefore, to find these points, which represent the coordinates in the initial frame, now in the primed frame at the synchronized time of ${\TTtPrime  } = 0$, we must propagate the points to that time, which ends up being ${\TTzPrime  } = \frac{ {\TTz } }{ {\TTgamma } }$, as shown in figure (\ref{fig: coordinate transform subfig_3}).
...
If we have the primed frame velocity much smaller than the speed of light, ($\TTv  \ll \TTc $), then we have ${\TTv }/{\TTc }\approx 0$ and therefore $\TTgamma  \approx 1$.
...
We can see when the $\TTgamma $-factor's influence starts to become significant from Figure (\ref{fig: Gamma Factor}).
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		...
		\end{axis}
	\end{tikzpicture}
	\caption{$\TTboldsymbolgamma $\textbf{-factor.} A diagram, showing the magnitude of $\TTgamma $ with increasing ratio of primed frame's speed to the speed of light, with ${\TTgamma }$ tending to infinity as ${\TTv }/{\TTc }\rightarrow 1$.}
...
If we want to know how the displacement between two coordinates ${\TTunderlineROne } = (\TTxOne ,\TTyOne ,\TTzOne )$ and ${\TTunderlineRTwo } = (\TTxTwo ,\TTyTwo ,\TTzTwo )$ transforms, we can take the difference of the transforms of both the coordinates as follows

\begin{equation}
	\begin{aligned}
		 & {\TTxPrime  }_2-{\TTxPrime  }_1 = \TTxTwo -\TTxOne                                                                                          \\
		 & {\TTyPrime  }_2-{\TTyPrime  }_1 = \TTyTwo -\TTyOne                                                                                          \\
		 & \TTzPrimeTwo -\TTzPrimeOne  = {\TTgamma } ( \TTzTwo - {\TTv } {\TTt }_2)-{\TTgamma } ( \TTzOne - {\TTv } {\TTt }_1)                                               \\
		 & {\TTtPrime  }_2-\TTtPrimeOne  = {\TTgamma } \bigg( {\TTt }_2-\frac{\TTv }{ {\TTc }^2} \TTzTwo  \bigg)-{\TTgamma } \bigg( {\TTt }_1-\frac{\TTv }{ {\TTc }^2} \TTzOne  \bigg)
	\end{aligned}
\end{equation}

rearranging and replacing the difference of the first and second of each coordinate with ${\TTDelta }{\TTunderlineR  } = ({\TTDelta }\TTx ,{\TTDelta }\TTy ,{\TTDelta }\TTz ) = (\TTxTwo -\TTxOne ,\TTyTwo -\TTyOne ,\TTzTwo -\TTzOne )$, giving the displacement transform as

\begin{equation}
	\label{eq: interval of Coordinates}
	\begin{aligned}
		 & \TTDelta  \TTxPrime  = {\TTDelta  \TTx }                                                   \\
		 & \TTDelta  {\TTyPrime  } = {\TTDelta  \TTy }                                                 \\
		 & \TTDelta  {\TTzPrime  } = {\TTgamma } ( {\TTDelta  \TTz }- {\TTv } \TTDelta  \TTt )                       \\
		 & {\TTDelta  \TTtPrime  } = {\TTgamma } \bigg( \TTDelta  \TTt -\frac{\TTv }{ {\TTc }^2} {\TTDelta  \TTz } \bigg)
	\end{aligned}
\end{equation}
...
\begin{equation}
	...
	\begin{aligned}
		 & {\TTd \TTxPrime  } = {\TTd \TTx }                                        \\
		 & {\TTd \TTyPrime  } = {\TTd \TTy }                                        \\
		 & {\TTd \TTzPrime  } = {\TTgamma } (\TTd \TTz - {\TTv }\TTd \TTt )                        \\
		 & {\TTd \TTtPrime  } = {\TTgamma } \bigg(\TTd \TTt -\frac{\TTv }{ {\TTc }^2} \TTd \TTz  \bigg)
	\end{aligned}
\end{equation}
...
The velocity at any instant is the infinitesimal change in position over the infinitesimal change in time $\frac{\TTd \TTunderlineR  }{\TTd \TTt }$.
...
\begin{equation}
	\begin{aligned}
		 & \TTuPrimex  = \frac{\TTd \TTxPrime  }{\TTd \TTtPrime  } = \frac{\TTd \TTx }{ {\TTgamma } \bigg(\TTd \TTt -\frac{\TTv \TTd \TTz }{ {\TTc }^2}\bigg)}                    \\
		 & \TTuPrimey  = \frac{\TTd \TTyPrime  }{\TTd \TTtPrime  } = \frac{\TTd \TTy }{ {\TTgamma } \bigg(\TTd \TTt -\frac{\TTv \TTd \TTz }{ {\TTc }^2}\bigg)}                    \\
		 & \TTuPrimez  = \frac{\TTd \TTzPrime  }{\TTd \TTtPrime  } = \frac{ {\TTgamma } (\TTd \TTz  - {\TTv }\TTd \TTt )}{ {\TTgamma } \bigg(\TTd \TTt -\frac{\TTv \TTd \TTz }{ {\TTc }^2}\bigg)}
	\end{aligned}
\end{equation}

now dividing the top and bottom of the fraction on the right hand side by ${\TTd \TTt }$ we can give the equations in their vector form as

\begin{equation}
	\mhl{
		\begin{aligned}
			 & \TTunderlineuPrime  = \dfrac{1}{ {\TTgamma }\left(1-\dfrac{\TTv }{ {\TTc }^2} {\TTuz }\right)}
			\begin{pmatrix}
				\TTux                                \\
				\TTuy                                \\
				{\TTgamma } \left( \TTuz  - {\TTv } \right) \\
			\end{pmatrix}
			\\
			 & \text{at position ${\TTunderlinerPrime  }$ and time ${\TTtPrime  }$}
		\end{aligned}
	}
...
\begin{equation}
	\TTunderlineuPrime  = \dfrac{1}{ {\TTgamma } } \dfrac{\TTunderlineu  + \Big[\dfrac{ {\TTgamma }-1}{\|\TTunderlinev \|^2}(\TTunderlineu \cdot \TTunderlinev )-{\TTgamma } \Big] \TTunderlinev  }{1-\dfrac{\TTunderlineu \cdot\TTunderlinev  }{ {\TTc }^2} }
\end{equation}

with ${\TTunderlinev  }$ being the frame's velocity in any generalized direction.

%███████████████████████████████████████████████████████████████████
%███████████████████████████████████████████████████████████████████
\section{Acceleration} \label{sect: Acceleration}

To find how the acceleration (known as the 3-acceleration) of a particle transforms for \hyperlink{def-observer}{observer}s in two different frames, we take the differential of the velocity \hyperlink{def-transform}{transform} in equation \eqref{eq: velocity transform} and use the differentiation rules for two generic functions ${\TTf }$ and ${\TTg }$: ${\TTd (\TTg \TTf )} = {\TTf } {\TTd \TTg } + {\TTg } {\TTd \TTf }$, and ${\TTd [{ {\TTf }({\TTg }(\TTx ))}]} = { {\TTd \TTg }(\TTx )} * {\TTd \TTf ({ {\TTg }(\TTx )})}$

\begin{equation}
	\begin{aligned}
		\TTd \TTunderlineuPrime  & \color{gray} = \dfrac{1}{ {\TTgamma }\left(1-\dfrac{\TTv }{ {\TTc }^2} {\TTuz }\right)}
		\begin{pmatrix}
			\TTd {\TTux }          \\
			\TTd {\TTuy }          \\
			{\TTgamma } \TTd {\TTuz } \\
		\end{pmatrix}
		+ \dfrac{ {\TTgamma } \dfrac{\TTv }{ {\TTc }^2} \TTd {\TTuz } }{ {\TTgamma }^2\left(1-\dfrac{\TTv }{ {\TTc }^2} {\TTuz }\right)^2}
		\begin{pmatrix}
			{\TTux }                              \\
			{\TTuy }                              \\
			{\TTgamma } \left( {\TTuz }- {\TTv } \right) \\
		\end{pmatrix}                                                          \\
		                  & \color{gray} = ...
		\\
		                  & = \dfrac{1}{ {\TTgamma }\left(1-\dfrac{\TTv }{ {\TTc }^2} {\TTuz }\right)^2}
		\begin{pmatrix}
			\TTd {\TTux } + \frac{\TTv }{ {\TTc }^2}( {\TTux } \TTd {\TTuz }-{\TTuz } \TTd {\TTux }) \\
			\TTd {\TTuy } + \frac{\TTv }{ {\TTc }^2}( {\TTuy } \TTd {\TTuz }-{\TTuz } \TTd {\TTuy }) \\
			\frac{1}{ {\TTgamma } } \TTd {\TTuz }                            \\
		\end{pmatrix}
	\end{aligned}
\end{equation}

Now, divide by ${\TTd \TTtPrime  }$ as we did for the velocity in the previous section, and we get the acceleration transform as

\begin{equation}
	\mhl{
		\begin{aligned}
			 & \TTunderlinea ' = \dfrac{1}{ {\TTgamma }^2\left(1-\dfrac{\TTv }{ {\TTc }^2} {\TTuz }\right)^3}
			\begin{pmatrix}
				{ {\TTa }_{\TTx } } + \frac{\TTv }{ {\TTc }^2}({\TTux } { {\TTa }_{\TTz } }-{\TTuz } { {\TTa }_{\TTx } }) \\
				{ {\TTa }_{\TTy } } + \frac{\TTv }{ {\TTc }^2}({\TTuy } { {\TTa }_{\TTz } }-{\TTuz } { {\TTa }_{\TTy } }) \\
				\frac{1}{ {\TTgamma } }{ {\TTa }_{\TTz } }
			\end{pmatrix}
			\\
			 & \text{at position ${\TTunderlinerPrime  }$ and time ${\TTtPrime  }$}
		\end{aligned}
...
\begin{equation}
	\TTunderlineaPrime  = \frac{1}{ {\TTgamma } ^3 \left(1-\frac{(\TTunderlineu \cdot \TTunderlinev )}{ {\TTc }^2}\right)^3}\left[ {\TTgamma } \left(1-\frac{(\TTunderlineu \cdot\TTunderlinev )}{ {\TTc }^2}\right)\TTunderlinea  + \frac{ {\TTgamma } (\TTunderlinea \cdot\TTunderlinev )}{ {\TTc }^2}\TTunderlineu  + (1-{\TTgamma } ) (\TTunderlinea \cdot\TThatunderlinev ) \cdot\TThatunderlinev \right]
\end{equation}

at position ${\TTunderlinerPrime  }$ and time ${\TTtPrime  }$.
...
The Resolution: This thought experiment serves as a "proof by contradiction" that information cannot travel faster than $\TTc $.
...
This introduced us to $\TTgamma $, which is a factor used throughout relativity to simplify equations.
...
The primed frame's axis will overlap with this initial frame's axis at your arbitrarily chosen starting time, that is when you define the time as ${\TTt }=0$ (the time of the overlap is also ${\TTtPrime  }=0$ in the primed frame).
...
And this is all the steps that are needed, but an easy mistake to make is to forget that ${\TTt }$ and ${\TTtPrime  }$ are the times from the overlap of the frame origins, in their respective frames.
...
When swapping between an initial frame to a primed frame, the speed of light does not change, that is $\TTcPrime =\TTc $.
We can equivalently express this using the light's velocity vector as $\|\TTmathbfcPrime \|=\|\TTmathbfc \|$.

In this chapter, we will see more invariant quantity related to the space-time coordinates ($\TTx ,\TTy ,\TTz ,\TTt $) and use this to find invariant properties associated with velocities, which will lead to the more abstract conservation relation between momentum, energy and mass.
...
An event in special relativity is defined by its spacial and time coordinates $\TTx $, $\TTy $, $\TTz $, and $\TTt $. This is called its space-time coordinates.
The change of the coordinates between two events are denoted as ${\TTDelta  \TTx }$, ${\TTDelta  \TTy }$, ${\TTDelta  \TTz }$, and ${\TTDelta  \TTt }$.

From these we could define a quantity called the space-time interval $\TTDelta  \TTS $ between two \hyperlink{def-event}{event}s in an inertial reference frame as:

\begin{equation}
	\label{eq: space-time interval}
	\mhl{
	\TTDelta  \TTS ^2 = (\TTc \TTDelta  \TTt )^2-{\TTDelta  \TTx }^2-{\TTDelta  \TTy }^2-{\TTDelta  \TTz }^2
	}
\end{equation}

The reason we created this new quantity, is because it turns out all inertial frames of reference agree on this interval of the two events, it is invariant between frames, $\TTDelta  \TTS  = \TTDelta  \TTSPrime $.
...
\begin{equation}
	\begin{aligned}
		\TTDelta  \TTSPrime ^2 & = (\TTc {\TTDelta  \TTtPrime  })^2-\TTDelta  \TTxPrime ^2-{\TTDelta  \TTyPrime  }^2-{\TTDelta  \TTzPrime  }^2                                                                                          \\
		              & \color{gray} = {\TTc }^2{\TTgamma } \bigg( \TTDelta  \TTt -\frac{\TTv }{ {\TTc }^2} {\TTDelta  \TTz } \bigg)^2-{\TTDelta  \TTx }^2-{\TTDelta  \TTy }^2-\TTDelta  ({\TTgamma } ( {\TTDelta  \TTz }- {\TTv } \TTDelta  \TTt ) )^2 \\
		              & \color{gray} = (\TTc \TTDelta  \TTt )^2-{\TTDelta  \TTx }^2-{\TTDelta  \TTy }^2-{\TTDelta  \TTz }^2                                                                                         \\
		              & = \TTDelta  \TTS ^2.
	\end{aligned}
\end{equation}
...
It is denoted as $\TTsquare $, and consists of the second order partial derivatives with respect to each coordinate.
It is defined in an initial frame as

\begin{equation}
	\label{eq: d Alembert operator}
	\TTsquare  = \frac{1}{\TTc ^2}\frac{\partial^2}{\partial \TTt ^2} - \left(\frac{\partial^2}{\partial \TTx ^2} + \frac{\partial^2}{\partial \TTy ^2} + \frac{\partial^2}{\partial \TTz ^2}\right)
\end{equation}

and in a primed frame as

\begin{equation}
	\TTsquarePrime  = \frac{1}{\TTc ^2} \frac{\partial^2}{\partial \TTtPrime ^2} - \frac{\partial^2}{\partial \TTxPrime ^2} - \frac{\partial^2}{\partial \TTyPrime ^2} - \frac{\partial^2}{\partial \TTzPrime ^2}
\end{equation}

In the following derivation we will show it is invariant by transforming the partial derivatives between reference frames and showing that the operator retains its form and that $\TTsquarePrime =\TTsquare $.
...
\begin{derivation}
	...
	\begin{equation}
		\begin{aligned}
			\frac{\partial \TTxPrime }{\partial \TTx } & = 1 & \frac{\partial \TTyPrime }{\partial \TTx } & = 0 & \frac{\partial \TTzPrime }{\partial \TTx } & = 0         & \frac{\partial \TTtPrime }{\partial \TTx } & = 0                     \\
			\frac{\partial \TTxPrime }{\partial \TTy } & = 0 & \frac{\partial \TTyPrime }{\partial \TTy } & = 1 & \frac{\partial \TTzPrime }{\partial \TTy } & = 0         & \frac{\partial \TTtPrime }{\partial \TTy } & = 0                     \\
			\frac{\partial \TTxPrime }{\partial \TTz } & = 0 & \frac{\partial \TTyPrime }{\partial \TTz } & = 0 & \frac{\partial \TTzPrime }{\partial \TTz } & = \TTgamma     & \frac{\partial \TTtPrime }{\partial \TTz } & = -\frac{\TTgamma  \TTv }{\TTc ^2} \\
			\frac{\partial \TTxPrime }{\partial \TTt } & = 0 & \frac{\partial \TTyPrime }{\partial \TTt } & = 0 & \frac{\partial \TTzPrime }{\partial \TTt } & = -\TTgamma  \TTv  & \frac{\partial \TTtPrime }{\partial \TTt } & = \TTgamma
		\end{aligned}
	\end{equation}
	...
	Which for $\frac{\partial}{\partial \TTt }$ is:

	\begin{equation}
		\frac{\partial}{\partial \TTt } = \frac{\partial \TTtPrime }{\partial \TTt }\frac{\partial}{\partial \TTtPrime } + \frac{\partial \TTxPrime }{\partial \TTt }\frac{\partial}{\partial \TTxPrime } + \frac{\partial \TTyPrime }{\partial \TTt }\frac{\partial}{\partial \TTyPrime } + \frac{\partial \TTzPrime }{\partial \TTt }\frac{\partial}{\partial \TTzPrime }
	\end{equation}

	and for $\frac{\partial}{\partial \TTz }$:

	\begin{equation}
		\frac{\partial}{\partial \TTz } = \frac{\partial \TTtPrime }{\partial \TTz } \frac{\partial}{\partial \TTtPrime } + \frac{\partial \TTxPrime }{\partial \TTz } \frac{\partial}{\partial \TTxPrime } + \frac{\partial \TTyPrime }{\partial \TTz } \frac{\partial}{\partial \TTyPrime } + \frac{\partial \TTzPrime }{\partial \TTz } \frac{\partial}{\partial \TTzPrime }
	\end{equation}
	...
	\begin{equation}
		\begin{aligned}
			\frac{\partial}{\partial \TTx } & = \frac{\partial}{\partial \TTxPrime }                                                             \\
			\frac{\partial}{\partial \TTy } & = \frac{\partial}{\partial \TTyPrime }                                                             \\
			\frac{\partial}{\partial \TTz } & = -\frac{\TTgamma  \TTv }{\TTc ^2} \frac{\partial}{\partial \TTtPrime } + \TTgamma  \frac{\partial}{\partial \TTzPrime } \\
			\frac{\partial}{\partial \TTt } & = \TTgamma  \frac{\partial}{\partial \TTtPrime } - \TTgamma  \TTv  \frac{\partial}{\partial \TTzPrime }
		\end{aligned}
	\end{equation}
	...
	\begin{equation}
		\begin{aligned}
			\frac{\partial^2}{\partial \TTt ^2} = \frac{\partial}{\partial \TTt } \cdot \frac{\partial}{\partial \TTt } & = \left(\TTgamma  \frac{\partial}{\partial \TTtPrime } - \TTgamma  \TTv  \frac{\partial}{\partial \TTzPrime }\right) \left(\TTgamma  \frac{\partial}{\partial \TTtPrime } - \TTgamma  \TTv  \frac{\partial}{\partial \TTzPrime }\right) \\
			                                                                                                & = \TTgamma ^2 \frac{\partial^2}{\partial \TTtPrime ^2} - 2\TTgamma ^2 \TTv  \frac{\partial^2}{\partial \TTtPrime \partial \TTzPrime } + \TTgamma ^2 \TTv ^2 \frac{\partial^2}{\partial \TTzPrime ^2}
		\end{aligned}
	\end{equation}

	\begin{equation}
		\begin{aligned}
			\frac{\partial^2}{\partial \TTz ^2} & = \left(-\frac{\TTgamma  \TTv }{\TTc ^2} \frac{\partial}{\partial \TTtPrime } + \TTgamma  \frac{\partial}{\partial \TTzPrime }\right) \left(-\frac{\TTgamma  \TTv }{\TTc ^2} \frac{\partial}{\partial \TTtPrime } + \TTgamma  \frac{\partial}{\partial \TTzPrime }\right) \\
			                                & = \frac{\TTgamma ^2 \TTv ^2}{\TTc ^4} \frac{\partial^2}{\partial \TTtPrime ^2} - \frac{2\TTgamma ^2 \TTv }{\TTc ^2} \frac{\partial^2}{\partial \TTtPrime \partial \TTzPrime } + \TTgamma ^2 \frac{\partial^2}{\partial \TTzPrime ^2}
		\end{aligned}
	\end{equation}

	\begin{equation}
		\begin{aligned}
			\frac{\partial^2}{\partial \TTx ^2} & = \frac{\partial^2}{\partial \TTxPrime ^2} \\
			\frac{\partial^2}{\partial \TTy ^2} & = \frac{\partial^2}{\partial \TTyPrime ^2}
		\end{aligned}
	\end{equation}
	...
	Let us first do this for just the $\TTt $ and $\TTz $ components $\left(\frac{1}{\TTc ^2}\frac{\partial^2}{\partial \TTt ^2} - \frac{\partial^2}{\partial \TTz ^2}\right)$:

	\begin{equation}
		\begin{aligned}
			\frac{1}{\TTc ^2}\frac{\partial^2}{\partial \TTt ^2} - \frac{\partial^2}{\partial \TTz ^2}
			 & = \frac{1}{\TTc ^2}\left(\TTgamma ^2 \frac{\partial^2}{\partial \TTtPrime ^2} - 2\TTgamma ^2 \TTv  \frac{\partial^2}{\partial \TTtPrime \partial \TTzPrime } + \TTgamma ^2 \TTv ^2 \frac{\partial^2}{\partial \TTzPrime ^2}\right)                                                              \\
			 & \quad - \left(\frac{\TTgamma ^2 \TTv ^2}{\TTc ^4} \frac{\partial^2}{\partial \TTtPrime ^2} - \frac{2\TTgamma ^2 \TTv }{\TTc ^2} \frac{\partial^2}{\partial \TTtPrime \partial \TTzPrime } + \TTgamma ^2 \frac{\partial^2}{\partial \TTzPrime ^2}\right)                                             \\
			 & = \frac{1}{\TTc ^2}\left(\TTgamma ^2 \frac{\partial^2}{\partial \TTtPrime ^2} + \TTgamma ^2 \TTv ^2 \frac{\partial^2}{\partial \TTzPrime ^2}\right) - \left(\frac{\TTgamma ^2 \TTv ^2}{\TTc ^4} \frac{\partial^2}{\partial \TTtPrime ^2} + \TTgamma ^2 \frac{\partial^2}{\partial \TTzPrime ^2}\right) \\
			 & = \frac{1}{\TTc ^2} \frac{\partial^2}{\partial \TTtPrime ^2} - \frac{\partial^2}{\partial \TTzPrime ^2}
		\end{aligned}
	\end{equation}
	...
	\begin{equation}
		\frac{1}{\TTc ^2}\frac{\partial^2}{\partial \TTt ^2} - \frac{\partial^2}{\partial \TTx ^2} - \frac{\partial^2}{\partial \TTy ^2} - \frac{\partial^2}{\partial \TTz ^2} = \frac{1}{\TTc ^2}\frac{\partial^2}{\partial \TTtPrime ^2} - \frac{\partial^2}{\partial \TTxPrime ^2} - \frac{\partial^2}{\partial \TTyPrime ^2} - \frac{\partial^2}{\partial \TTzPrime ^2}
	\end{equation}
...
\begin{derivation}
	...
	We can see from equation \eqref{eq: space-time interval}, that if a particle $\TTq $, is moving in an inertial reference frame, we can define the space-time interval of its coordinates in this frame as

	\begin{equation}
		\TTDelta  \TTSq ^2 = (\TTc \TTDelta  {\TTt }_\TTq )^2-\TTDelta  \TTxq ^2-{\TTDelta  \TTyq }^2-{\TTDelta  \TTzq }^2
	\end{equation}
	...
	Now we can divide this space-time interval by the square of the proper time that passes for the particle in this interval, ${\TTDelta \TTtau }$ (the interval in time that passes in the particles rest frame).
	...
	\begin{equation}
		\left(\frac{\TTd \TTSq }{\TTd \TTtau }\right)^2 = \left(\TTc \frac{ { {\TTd \TTt }_{\TTq } } }{\TTd \TTtau }\right)^2-\left(\frac{\TTd \TTxq }{\TTd \TTtau }\right)^2-\left(\frac{\TTd \TTyq }{\TTd \TTtau }\right)^2-\left(\frac{\TTd \TTzq }{\TTd \TTtau }\right)^2
	\end{equation}

	From the time dilation equation \eqref{eq: time dilation}, we have $\TTd \TTtau  = \TTd \TTtq  / \TTgammau  $ with $\TTgammau  = \frac{1}{\sqrt{1-\frac{\TTuq ^2}{ {\TTc }^2} } }$.
	...
	\begin{equation}
		...
		\begin{aligned}
			\left(\frac{\TTd \TTSq }{\TTd \TTtau }\right)^2 & = \TTgammau ^2\left(\TTc \frac{ { {\TTd \TTt }_{\TTq } } }{ { {\TTd \TTt }_{\TTq } } }\right)^2-\TTgammau ^2\left(\frac{\TTd \TTxq }{ { {\TTd \TTt }_{\TTq } } }\right)^2-\TTgammau ^2\left(\frac{\TTd \TTyq }{ { {\TTd \TTt }_{\TTq } } }\right)^2-\TTgammau ^2\left(\frac{\TTd \TTzq }{ { {\TTd \TTt }_{\TTq } } }\right)^2 \\
			                                  & = {\TTc }^2\TTgammau ^2 \left( 1-\frac{ { {\TTu }_{\TTp \TTx } }^2 + { {\TTu }_{\TTp \TTy } }^2 + { {\TTu }_{\TTp \TTz } }^2}{ {\TTc }^2} \right)                                                                                                                        \\
			                                  & = {\TTc }^2\TTgammau ^2 \left( 1-\frac{\TTuq ^2}{ {\TTc }^2} \right)                                                                                                                                                             \\
			                                  & = {\TTc }^2
		\end{aligned}
	\end{equation}
	...
	Now we could multiply both sides of the third line in equation \eqref{eq: particles space-time interval} by the invariant quantity ${\TTm }^2{\TTc }^2$ where $\TTm $ is the invariant rest mass of the particle, so that we get

	\begin{equation}
		\begin{aligned}
			\TTm ^2 {\TTc }^2 \left(\frac{\TTd \TTSq }{\TTd \TTtau }\right)^2 & = \TTm ^2{\TTc }^4\TTgammau ^2 \left( 1-\frac{\TTuq ^2}{ {\TTc }^2} \right) \\
			({\TTm }{\TTc }^2)^2                                & = \TTgammau ^2 \TTm ^2 \TTc ^4-\TTgammau ^2 \TTm ^2 \TTuq ^2 {\TTc }^2
		\end{aligned}
	\end{equation}

	were $\left(\frac{\TTd \TTSq }{\TTd \TTtau }\right)^2={\TTc }^2$ was substituted in to get the final equation.
...
\begin{equation}
	...
	(\TTm  {\TTc }^2)^2  = (\TTgammau  \TTm  \TTc ^2)^2 - (\TTgammau  \TTm  \TTuq  {\TTc })^2
\end{equation}
...
The equation \eqref{eq: energy-momentum derivation}, shows an invariant quantity with the units of energy squared $(\TTm  {\TTc }^2)^2$, from this we have the energy term $\TTm  {\TTc }^2$ itself being invariant; it is an energy that is the same in all frames of reference.
...
\begin{derivation}
	...
	\begin{equation}
		...
		\left( \TTgammau  \TTm  {\TTc }^2 \right)^2 = ({\TTm }{\TTc }^2)^2 + \left( \TTgammau  {\TTm }{\TTuq }{\TTc }\right)^2
	\end{equation}

	for the energy term in the brackets on the left hand side we can use what is called the Taylor expansion to write $\TTgammau $ as a convergent series:

	\begin{equation}
		\TTgammau  \equiv 1 + \frac{1}{2}\frac{\TTuq ^2}{ {\TTc }^2} + \frac{3}{8}\frac{\TTuq ^4}{\TTc ^4} + ...
	\end{equation}
	...
	\begin{equation}
		\begin{aligned}
			\TTgammau  {\TTm }{\TTc }^2 & = {\TTm }{\TTc }^2  \left(1 + \frac{1}{2}\frac{\TTuq ^2}{ {\TTc }^2} + \frac{3}{8}\frac{\TTuq ^4}{\TTc ^4} + ...
			\right)                                                                                                         \\
			                    & = {\TTm }{\TTc }^2 + \frac{1}{2}{\TTm }{\TTuq }^2 + \frac{3}{8}\TTm \frac{\TTuq ^4}{ {\TTc }^2} + ...
		\end{aligned}
	\end{equation}
	...
	The second quantity is the classical kinetic energy formula, the rest may be seen as higher-order kinetic energy terms, that are not seen in classical physics because they are very small for ${\TTuq }<<{\TTc }$.
	...
	\begin{equation}
		...
		{\TTE } = \TTgammau  {\TTm }{\TTc }^2
	\end{equation}

	and the rest mass energy from last section denoted as $\TTEZero $, allowing us to write equation \eqref{eq: total relativistic energy} as

	\begin{equation}
		\label{eq: energy momentum deriving momentum term}
		\TTE ^2 = \TTEZero ^2 + (\TTgammau {\TTm }{\TTuq }{\TTc })^2
	\end{equation}

	Now let us look at this last energy term in the brackets, along with the Taylor expansion of $\TTgammau $, giving

	\begin{equation}
		\begin{aligned}
			\TTgammau  {\TTc }{\TTm }{\TTuq } & = {\TTc }{\TTm }{\TTuq } \left(1 + \frac{1}{2}\frac{\TTuq ^2}{ {\TTc }^2} + \frac{3}{8}\frac{\TTuq ^4}{\TTc ^4} + ...
			\right)                                                                                                               \\
			                       & ={\TTc }\left( {\TTm }{\TTuq } + \frac{1}{2}\TTm  \frac{\TTuq ^3}{ {\TTc }^2} + \frac{3}{8}\TTm \frac{\TTuq ^5}{\TTc ^4} + ...
			\right)
		\end{aligned}
	\end{equation}

	At small particle velocities this last term is approximately $({\TTm }{\TTuq }){\TTc }$ with ${\TTm }{\TTuq }$ being the classical momentum, it seems like this energy, could be the relativistic version of the particle's momentum, times the speed of light.
...
\begin{equation}
	\begin{aligned}
		\sum \TTpafter  & = \sum \TTpbefore  \\
		\sum \TTEafter  & = \sum \TTEbefore
	\end{aligned}
\end{equation}
...
We will start with the previous definitions for the momentum and total energy, $\TTmathbfp  = \TTgammau  \TTm  \TTmathbfu $ and $\TTE  = \TTgammau  \TTm  \TTc ^2$.
...
We will also make use of an identity for $\TTgammauPrime $ given by

\begin{equation}
	\label{eq: gamma u prime}
	\TTgammauPrime  = \TTgammau  \TTgammav  \left(1 - \frac{\TTuz  \TTv }{\TTc ^2}\right)
\end{equation}

I will let you check this identity yourself, as it is just the substitution of the transform equation of $\TTuPrime $ into the gamma function with some rearrangment.

%█████████████
\subsubsection{Y-Component}

Using equation \eqref{eq: velocity transform} and equation \eqref{eq: gamma u prime} on the relativistic momentum's y-component in the primed frame $\TTpPrimey  = \TTgammauPrime  \TTm  \TTuPrimey $, gives:

\begin{equation}
	\begin{aligned}
		\TTpPrimey  &= \left[ \TTgammau  \TTgammav  \left(1 - \frac{\TTuz  \TTv }{\TTc ^2}\right) \right] \TTm  \left[ \frac{\TTuy }{\TTgammav (1 - \frac{\TTuz  \TTv }{\TTc ^2})} \right] \\
		\TTpPrimey  &= \TTgammau  \TTm  \TTuy   \\
		\TTpPrimey  &= \TTpy
	\end{aligned}
\end{equation}
...
Now doing the same for the z-component $\TTpPrimez  = \TTgammauPrime  \TTm  \TTuPrimez $, we get:

\begin{equation}
\begin{aligned}
	\TTpPrimez  & = \left[ \TTgammau  \TTgammav  \left(1 - \frac{\TTuz  \TTv }{\TTc ^2}\right) \right] \TTm  \left[ \frac{\TTuz  - \TTv }{1 - \frac{\TTuz  \TTv }{\TTc ^2} } \right] \\
	\TTpPrimez  & = \TTgammau  \TTgammav  \TTm  (\TTuz  - \TTv )                                                                                              \\
	\TTpPrimez  & = \TTgammav  (\TTgammau  \TTm  \TTuz ) - \TTgammav  \TTv  (\TTgammau  \TTm )
\end{aligned}
\end{equation}
...
Using the relativistic momentum in the initial frame $\TTpz  = \TTgammau  \TTm  \TTuz $ and the equation for the total energy that we described in previous section $\TTE  = \TTgammau  \TTm  \TTc ^2$, we obtain:

\begin{equation}
	\TTpPrimez  = \TTgammav  \left( \TTpz  - \frac{\TTv  \TTE }{\TTc ^2} \right)
\end{equation}
...
\begin{equation}
	\begin{aligned}
		\TTDelta  \TTpPrimez  &= \TTgammav  \left( \TTpTwoz  - \frac{\TTv  \TTETwo }{\TTc ^2} \right) - \TTgammav  \left( \TTpOnez  - \frac{\TTv  \TTEOne }{\TTc ^2} \right) \\
		&= \TTgammav  \left( \TTDelta  \TTpz  - \frac{\TTv  \TTDelta  \TTE }{\TTc ^2} \right)
	\end{aligned}
\end{equation}
...
This shows that taking the $\TTgammau  \TTm  \TTuq $ term as being the relativistic momentum and the term $\TTgammau  \TTm  \TTc ^2$ as the total energy is consistent with special relativity and the laws of the conservation of momentum and energy.

Now what about for light itself, when $\TTgamma $ is infinitly large, can we still get the energy-momentum relation to work?
...
\begin{equation}
		\TTE ^2 = (\TTm \TTc ^2)^2 + (\TTgammau {\TTm }{\TTuq }{\TTc })^2
\end{equation}

For the speed of light we have $\TTgammau $ being infinitely large, so if the photon had a non zero rest mass, the second term would be infinite, therefor giving the total energy as infinite, which is not possible.
...
\begin{equation}
	\TTEph  = (\TTgammac {\TTmph  }{\TTc }){\TTc }
\end{equation}

now if we were to get a finite energy from this, we would require $\TTgammac {\TTmph  }$ (which is an infinite value multiplied by and infintesmal) to be finite for it to give a meaningful energy of the photon, we previously had the term $\TTp =\TTgammau {\TTm }$ being the relativistic momentum of a particle, so we could take this as the momentum of a photon aswell, and we could think of $\TTmeff =\TTgammac {\TTmph  }$ as an effective mass that a photon has, and the momentum of the photon as $\TTpph =\TTmeff {\TTc }$.
This would give the momentum for light in terms of its energy as

\begin{equation}
	\mhl{
	 {\TTpph  }= \frac{\TTEph  }{\TTc }
	}
\end{equation}

with $\TTpph $ and $\TTEph $ being the momentum and energy of the photon.
...
\begin{equation}
	...
	\mhl{
			\TTE ^2  = (\TTm \TTc ^2)^2 + (\TTp \TTc )^2
	}
\end{equation}

with momentum being $\TTp  = \TTgammau  {\TTm }{\TTuq }$ for a particle with mass and writen more meaningfully for a photon as $\TTp =\TTE /\TTc $.
...
If you still want to consider the posibility of a particle moving at speeds faster than light speed, it would lead to $\TTgamma $ being imaginary (squareroot of a negaive number).
...
\begin{figure}[ht]
	\begin{subfigure}[b]{.49\textwidth}
		...
		\begin{tikzpicture}[scale = 5] %[,tdplot_main_coords]
			...
			\draw[black, thick,->] (0,0,0)--(0.4,0,0) node[anchor = north east]{${\TTy }$};
			\draw[black, thick,->] (0,0,0)--(0,0.4,0) node[anchor = north east]{${\TTz }$};
			\draw[black, thick,->] (0,0,0)--(0,0,0.55) node[anchor = south east] {${\TTx }$};
			\draw[gray,dashed, thick,->] (0,0,0)--(0.2,0,0) node[anchor = north east]{${\TTyPrime  }$};
			\draw[gray,dashed, thick,->] (0,0,0)--(0,0.2,0) node[anchor = north east]{${\TTzPrime  }$};
			\draw[gray,dashed, thick,->] (0,0,0)--(0,0,0.35) node[anchor = south] {${\TTxPrime  }$};
			\draw[gray, thick,,->>] (-0.15,0,0)--(-0.15,0.1,0) node[anchor = north east]{${\TTv }$};
			\draw[->] (0,0.25/2,0) to[bend left = 45] (0.19/2,0.18/2,0);
			\node at (0.08,0.17,0) {${\TTthetaret  }$};
			\draw[red, thick, dotted,<-] (0.02,0.02,0)--(0.47,0.46,0); % Reduced by 0.02
			\fill[blue] (0.49,0.48,0) circle (0.4pt);
			\node[blue] at (0.67,0.48,0) {\text{${\TTunderlinerret  }$} }; % Reduced by 0.02
			\draw[gray,dashed] (0.49,0.48,0)--(0.49,0.65,0); % Reduced by 0.02
			\draw[red,->] (0.49,0.58,0) arc (90:-135:0.1);
			\node[red] at (0.54,0.47,0) {${\TTalpha } $};
		\end{tikzpicture}
	...
	\begin{subfigure}[b]{.49\textwidth}
		...
		\begin{tikzpicture}[scale = 5] %[,tdplot_main_coords]
			...
			\draw[gray,dashed, thick,->] (0,0.7,0)--(0.2,0.7,0) node[anchor = north east]{${\TTy }$};
			\draw[gray,dashed, thick,->] (0,0.7,0)--(0,0.9,0) node[anchor = north east]{${\TTz }$};
			\draw[gray,dashed, thick,->] (0,0.7,0)--(0,0.7,0.35) node[anchor = south] {${\TTx }$};
			\draw[gray,dashed, thick,->] (0,0,0)--(0.2,0,0) node[anchor = north east]{${\TTy }$};
			\draw[gray,dashed, thick,->] (0,0,0)--(0,0.2,0) node[anchor = north east]{${\TTz }$};
			\draw[gray,dashed, thick,->] (0,0,0)--(0,0,0.35) node[anchor = south] {${\TTx }$};
			\draw[thick,->] (0,0,0)--(0.4,0,0) node[anchor = north east]{${\TTyPrime  }$}; % Reduced by 0.02
			\draw[thick,->] (0,0,0)--(0,0.4,0) node[anchor = north east]{${\TTzPrime  }$};
			\draw[thick,->] (0,0,0)--(0,0,0.55) node[anchor = south east] {${\TTxPrime  }$};
			...
			\fill[gray] (0.47,0.9,0) circle (0.4pt) node[right] {\text{${\TTunderlinerPrimeret  }$} }; % Reduced by 0.02
			\fill[blue] (0.47,0.2,0) circle (0.4pt) node[right] {\text{${\TTunderlinerPrimetPrimeZero  }$} }; % Reduced by 0.02
			\draw[gray, thick,->>] (0,0.58,0)--(0,0.49,0) node[midway,right]{$-{\TTv }$};
			\draw[blue, thick,->>] (0.47,0.58,0)--(0.47,0.49,0) node[midway,right]{$-{\TTv }$}; % Reduced by 0.02
			...
			\node at (0.065,0.29,0) {${\TTthetaPrimeret  }$};
			%\node[gray] at (0.67,0.91,0) {${\underline{r}'_{ret} }$}; % Reduced by 0.02
		\end{tikzpicture}
		\caption{\hyperlink{def-Primed-Frame}{primed frame} }
		% \label{fig:sub2}
	\end{subfigure}
	\caption{\textbf{Retarded coordinates.} Diagrams of a source of light that represents, in the source's rest frame, the coordinate ${\TTunderliner  } = {\TTunderlinerret  }$ (blue) emitting light (red) at an angle ${\TTalpha }$ towards the origin, which is received by an \protect\hyperlink{def-observer}{observer} positioned at the origin at time ${\TTt } = {\TTtPrime  } = 0$ with the \protect\hyperlink{def-observer}{observer} and source at rest in the first inertial frame (left) and then shown moving in a \protect\hyperlink{def-Primed-Frame}{primed frame} at speed $-{\TTv }$ in the z-direction (right). In the primed frame, the past, retarded position of the rest frame and also the source's coordinate ${\TTunderlinerPrimeret  }$ at its retarded time are shown in gray, with the source's current position $\TTunderlinerPrimetPrimeZero $ at time ${\TTtPrime  }=0$ shown in blue. The path light takes is shown in red. The corresponding axis in each frame is also shown in black. The angle of the retarded position from the origin is ${\TTthetaret  }$ and ${\TTthetaPrimeret  }$ respectively.}
	\label{fig: Retarded field}
\end{figure}
%███████████

As shown in the diagram, if we take the \hyperlink{def-proper-frame}{rest frame} of an \hyperlink{def-observer}{observer} and a source that emits a pulse of light from its position at ${\TTunderliner  }$ so that it is received by the \hyperlink{def-observer}{observer} at the origin, at time ${\TTt } = 0$.
The retarded time the pulse of light was emitted is

\begin{equation}
	{ {\TTt }_{\TTr \TTe \TTt } } =-\frac{\|\TTunderliner \|}{\TTc },
\end{equation}

which is the negative of the time it took for light to propagate along ${\TTunderliner  }$ to the origin.
...
\begin{equation}
	\begin{aligned}
		\TTunderlinerPrimeret      & = {\TTc }{ {\TTt }_{\TTr \TTe \TTt } }
		\begin{pmatrix}
			0                                                \\
			\sin{\TTalpha }                                     \\
			{\TTgamma } \left( \cos{\TTalpha }-\frac{\TTv }{\TTc } \right) \\
		\end{pmatrix}
		=
		\begin{pmatrix}
			0                                             \\
			{\TTy }                                           \\
			{\TTgamma } \left( {\TTz }-{\TTv } { { {\TTt }_{\TTr \TTe \TTt } } } \right) \\
		\end{pmatrix}
		\\
		\text{at retarded time } &
		\\ { {\TTt }'_{\TTr \TTe \TTt } } & = {\TTgamma } \left(1-\frac{\TTv }{\TTc } \cos{\TTalpha } \right) { { {\TTt }_{\TTr \TTe \TTt } } }
	\end{aligned}
\end{equation}

If instead we use the angle of the retarded coordinate from the z-axis, $\TTthetaret  = \TTalpha  - \TTpi $, we have that $\cos{\TTalpha }=\cos{(\TTthetaret +\TTpi )}=-\cos{\TTthetaret  }$ and therefor the retarded time can be given as

\begin{equation}
	\\ { {\TTt }'_{\TTr \TTe \TTt } } = {\TTgamma } \left(1 + \frac{\TTv }{\TTc } \cos{\TTthetaret  } \right) { { {\TTt }_{\TTr \TTe \TTt } } }
\end{equation}

With the light being received at the origin of the \hyperlink{def-Primed-Frame}{primed frame} axis at time ${\TTtPrime  } = 0$.

The coordinate transform here for retarded coordinates is the normal coordinate transform, except the time transform simplifies and allows us to take the retarded time out of brackets with just the factor $\text{\AA}={\TTgamma } \left(1-\frac{\TTv }{\TTc } \cos{\TTalpha } \right) = {\TTgamma } \left(1 + \frac{\TTv }{\TTc } \cos{\TTthetaret  } \right)$ now in front of it.
...
We can use this factor to simplify equations like we did with $\TTgamma $ previously.
...
If you wanted to find the position of where the source is at the same time as the observer, $\TTtPrime =0$, we need to propagate this from its retarded position and time, to this time of zero.
So we need to take into account that the source is moving at $\TTu  = -\TTv $ and the retarded time is the negative of the time that we want to pass, for the source to move to its current position.
...
\begin{equation}
	...
	\TTunderlinerPrimetPrimeZero  = \TTunderlinerPrimeret  + \TTunderlinev \TTtPrimeret  = {\TTc }{ {\TTt }_{\TTr \TTe \TTt } }
	\begin{pmatrix}
		0                               \\
		\sin{\TTalpha }                    \\
		\frac{1}{ {\TTgamma } } \cos{\TTalpha } \\
	\end{pmatrix}
\end{equation}

So for a stationary source in the rest frame at $\TTt =0$, we have this position in the primed frame at $\TTtPrime =0$.
...
\begin{figure}[H]
	...
	\begin{subfigure}{0.45\textwidth}
		...
		\caption{primed frame (${\TTtPrimeret  }$)}
		\label{fig: retarded coordinate transform subfig_2}
	\end{subfigure}
	\caption{\textbf{Retarded coordinate transform.} A diagram, showing an initial small grid of points, representing some position coordinates that are at rest in their proper frame (left), with an \protect\hyperlink{def-observer}{observer} at the origin, using the retarded time ${\TTtret  } = \frac{\|\TTunderliner \|}{\TTc }$ for these coordinates, we can transform these to their retarded positions in a primed frame using the coordinate transform, with the original frames coordinates now moving at velocity $-{\TTv } =-0.9$, the initial frames coordinates are transformed to where they would be seen to be at by an \protect\hyperlink{def-observer}{observer} at the origin with the \protect\hyperlink{def-observer}{observer}s time ${\TTtPrime  } = 0$, these positions are due to the delay in the light from these points taking time to get to the origin, they are also known as the \protect\hyperlink{def-retarded-position}{retarded position}s.}
...
\begin{figure}[H]
	...
	\caption{\textbf{Volume density of initial frame's retarded coordinates in primed frame}. This shows the density of the initial frames coordinates when they are in their retarded positions in the primed frame, with $\TTv =0.9$, with the color scale showing the density.}
	\label{fig: Density of initial frame's retarded coordinates in primed frame}
\end{figure}
%███████████

If we are at rest in the initial frame and we see the points as they are shown the initial frame in figure (\ref{fig: retarded coordinate transform subfig_1}), then if there was another observer overlapping with us moving at $\TTunderlinev $, they would see all the points distributed as they are shown in their primed frame in figure (\ref{fig: retarded coordinate transform subfig_2}).
...
If we have a moving particle positioned at $\TTunderliner $, then in a infinitesimal time ${\TTd \TTt }$ it will move

\begin{equation}
	\TTd \TTunderliner = \TTunderlineu  {\TTd \TTt }
\end{equation}

to position $\TTunderliner  + \TTd \TTunderliner $, as shown in the Diagram

*** r is actually retarded position of particle
*** do i need to use $\TTrret $, i.e. what time are we using for this derivation

*** or do we say we are actually looking at the actually positions but look at what these are used at retarded times with respect to the observer at origin instead of actual times, and using $\TTunderlinerret =\TTunderliner $ and $\TTd \TTunderlinerret =\TTd \TTunderliner $
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[scale = 5]
		...
		\fill[blue] (0.4,0.4,0) circle (0.4pt) node[anchor = south] {\text{${\TTunderliner  }$} };
		\fill[blue] (0.5,0.3,0) circle (0.4pt) node[anchor = west] {\text{${\TTunderliner  } + {\TTd \TTunderliner  }$} };
	\end{tikzpicture}
	\caption{\textbf{Apparent velocity.} Diagram showing a particle (blue) moving from $\TTunderliner $ by a displacement of $\TTd \TTunderliner =\TTunderlineu \TTd \TTt $ with the paths that light has to travel to get to the origin from both these positions(red). Leading to a different retarded time between the positions than the actual time between them. As to due to the particle moving to a different position, the light may also have to travel a different distance, which also introduces an additional time difference. This gives a different Apparent velocity to the actual velocity.}
	\label{fig: Apparent velocity}
\end{figure}
%███████████

The difference between the retarded times of light from each position reaching the origin, is the time that light takes to get from position  $\TTunderliner  + \TTd \TTunderliner $ minus the time it takes to get from position $\TTunderliner $, plus the time it takes for the particle to move to the new position $\TTd \TTt $. Which is


\begin{equation}
	\TTd \TTtret  =   \frac{\|\TTunderliner  + \TTd \TTunderliner \|}{\TTc } -  \frac{\|\TTunderliner \|}{\TTc } + \TTd \TTt
\end{equation}

since $\TTd \TTunderliner $ is infinitesmally small we can simplify this expression by using some approximations as shown in the following

\begin{equation}
	\begin{aligned}
		\|\TTunderliner  + \TTd \TTunderliner \| & \color{gray} =  \sqrt{(\TTunderliner  + \TTd \TTunderliner ) \cdot (\TTunderliner  + \TTd \TTunderliner )}                                       \\
		                                   & \color{gray} = \sqrt{\TTunderliner  \cdot \TTunderliner  + 2 \TTunderliner  \cdot \TTd \TTunderliner  + \TTd \TTunderliner  \cdot \TTd \TTunderliner  } \\
		                                   & \color{gray} \approx \sqrt{\TTunderliner  \cdot \TTunderliner  + 2 \TTunderliner  \cdot \TTd \TTunderliner  }                                 \\
		                                   & \color{gray} \approx \|\TTunderliner \| \sqrt{1 + \frac{2 \TTunderliner  \cdot \TTd \TTunderliner  }{\TTunderliner  \cdot \TTunderliner  } }     \\
		                                   & \color{gray} \text{using the Taylor expansion $\sqrt{1 + \TTx } \approx 1 + \frac{\TTx }{2} \text{for } \TTx  \ll 1$:}                           \\
		                                   & \color{gray} \approx \|\TTunderliner \| \left(1 + \frac{\TTunderliner  \cdot \TTd \TTunderliner  }{\|\TTunderliner \|^2}\right)               \\
		                                   & \color{gray} \approx \|\TTunderliner \|  + \frac{\TTunderliner  \cdot \TTd \TTunderliner  }{\|\TTunderliner \|}                               \\
		                                   & \approx \|\TTunderliner \|  + \TTunderlinehatr  \cdot \TTd \TTunderliner
	\end{aligned}
\end{equation}

so for the infinitesimal change we have $\|\TTunderliner  + \TTd \TTunderliner \|= \|\TTunderliner \|   + \TTunderlinehatr  \cdot \TTd \TTunderliner $, and we have the perceived time that has passed at the origin as

\begin{equation}
	\begin{aligned}
		\TTd \TTtret  & \color{gray} =   \TTd \TTt  + \frac{\TTunderlinehatr  \cdot \TTd \TTunderliner  }{\TTc }  \\
		         & \color{gray} = \TTd \TTt  + \frac{\TTunderlinehatr  \cdot \TTunderlineu  \TTd \TTt }{\TTc }  \\
		         & = \left( 1 + \frac{\TTunderlineu  \cdot \TTunderlinehatr  }{\TTc } \right) \TTd \TTt
	\end{aligned}
\end{equation}
...
\begin{equation}
	\begin{aligned}
		\TTunderlineuret  & \color{gray}= \frac{\TTd \TTunderliner  }{\TTd \TTtret  }                                                                \\
		                    & \color{gray}= \frac{\TTd \TTunderliner  }{\left( 1 + \frac{\TTunderlineu  \cdot \TTunderlinehatr  }{\TTc } \right) \TTd \TTt } \\
		                    & = \frac{\TTunderlineu  }{1 + \frac{\TTunderlineu  \cdot \TTunderlinehatr  }{\TTc } }
	\end{aligned}
\end{equation}
...
For something moving at the speed of light, we have $ 0.5 \TTc  \leq \|\TTunderlineuret \| \leq \infty $ depending on the direction it is moving.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		\begin{axis}[
				domain=-0.8:0.99,
				samples=100,
				xlabel={$\TTunderlinec \cdot\TTunderlinehatr $},
				ylabel={$\|\TTunderlinecret \|$},
				grid=major,
				grid style={dashed, gray!30},
				axis lines=middle,
				xmin=-1, xmax=1.01,
		...
		\end{axis}
	\end{tikzpicture}
	\caption{\textbf{Apparent speed of light.} A diagram, showing the magnitude of the apparent velocity of light $\|\TTunderlinecret \|$ depending on the direction of light relative to the radial direction from the observer, that is $-1 \leq \TTunderlinec \cdot\TTunderlinehatr  \leq 1$.}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[scale = 3,tdplot_main_coords]
		...
		% axes
		\coordinate (O) at (0,0,0);
		\draw[thick,->] (O)--(1,0,0) node[anchor = north east]{${\TTx }$};
		\draw[thick,->] (O)--(0,0.92,0) node[anchor = north west]{${\TTy }$};
		\draw[thick,->] (O)--(0,0,1) node[anchor = south]{${\TTz }$};
		% vectors
		\tdplotsetcoord{P}{\rvec}{\thetavec}{\phivec}
		\draw[dashed] (O)--(Pxy);
		\draw[dashed] (P)--(Pxy);
		\draw[dashed] (Py)--(Pxy);
		\draw[-stealth,thick, red] (O)--(P) node[above right] {$\TTunderliner $} node[black, midway, above] {$\TTr $};
		% arcs
		\tdplotdrawarc[->]{(O)}{0.2}{0}{\phivec}{anchor = north}{$\TTphi $}
		\tdplotsetthetaplanecoords{\phivec}
		\tdplotdrawarc[->,tdplot_rotated_coords]{(0,0,0)}{0.3}{0}{\thetavec}{}{}
		\node at (0,0.038,0.17) {$\TTtheta $};
		% sphere
		\shadedraw[tdplot_screen_coords,ball color = gray, opacity = 0.3] (0,0) circle (\rvec);
		\draw[fill = gray,opacity = 0.3] (0,0) circle (\rvec);
	\end{tikzpicture}
	\caption{\textbf{Spherical polar coordinates.} Diagram of the Cartesian $\TTx ,\TTy ,\TTz $-axis showing a position vector $\TTunderliner $ (red) with its associated spherical polar coordinates $(\TTr ,\TTtheta ,\TTphi )$, where $\TTtheta $ is the angle from the $\TTz $-axis, $\TTphi $ is the angle from the $\TTx $-axis in the x-y plane, and $\TTr $ is the magnitude.}
\end{figure}
%███████████

The figure shows, for a given position $\TTunderliner $ we have the Cartesian coordinates $({\TTx },{\TTy },{\TTz })$, but we can also write this position in terms of spherical polar coordinates $(\TTr ,\TTtheta ,\TTphi )$.
Where the angles from the Z-Axis and X-axis are $\TTtheta $ (between 0 and $\TTpi $) and $\TTphi $ (between 0 and $2\TTpi $) respectively, and ${\TTr }$ is the displacement to the position from the origin.
...
\begin{equation}
	...
	\mhl{
		...
		= \TTr
		\begin{pmatrix}
			\cos{\TTphi }\sin{\TTtheta } \\
			\sin{\TTphi }\sin{\TTtheta } \\
			\cos{\TTtheta }           \\
		\end{pmatrix}
	}
\end{equation}
...
\begin{equation}
	\begin{aligned}
		\TTr  = \|\TTunderliner \| & =\sqrt{\TTx ^2 + \TTy ^2 + \TTz ^2}      \\
		\cos\TTtheta             & = \frac{\TTz }{\TTr }                \\
		\cos\TTphi               & = \frac{\TTx }{\sqrt{\TTx ^2 + \TTy ^2} }
	\end{aligned}
\end{equation}
...
From here we will start referencing explicitly the rest frame of a source, and a frame with the source moving at a speed $\TTu $.
...
Light emitted at a speed $\TTc $ from a source at rest, at an angle $\TTalphaZero $ from the z-axis and $\TTvarphiZero $ from the x-axis, has the velocity written in spherical polar coordinates as

\begin{equation}
	\TTunderlinecZero   = {\TTc } \cdot {\TTunderlinehatcZero  }= {\TTc }
	\begin{pmatrix}
		\cos{\TTvarphiZero }\sin{\TTalphaZero } \\
		\sin{\TTvarphiZero }\sin{\TTalphaZero } \\
		\cos{\TTalphaZero }                \\
	\end{pmatrix},
\end{equation}

then for the frame with the source moving at speed $\TTu $ in the z-direction, we have this frame moving at $\TTv =-\TTu $ relative to the rest frame.
...
\begin{equation}
	...
	\mhl{
		\TTunderlinec  = \TTc \TTc ^2 + \TTm  \meow \TTo \TTw
		\begin{pmatrix}
			\cos\TTvarphi \sin\TTalpha  \\
			\sin\TTvarphi \sin\TTalpha  \\
			\cos\TTalpha             \\
		\end{pmatrix}
		=
		\dfrac{\TTc }{ {\TTgamma }\left(1+\frac{\TTu }{\TTc } \cos\TTalphaZero \right)}
		\begin{pmatrix}
			\cos\TTvarphiZero \sin\TTalphaZero                         \\
			\sin\TTvarphiZero \sin\TTalphaZero                         \\
			{\TTgamma } \left( \cos\TTalphaZero +\frac{\TTu }{\TTc } \right) \\
		\end{pmatrix}
	}
\end{equation}

Using a bit of trigonometry to find the z-components in terms of sines instead of cosines, and substituting them into the other two components, you can find that $\TTvarphi =\TTvarphiZero $.
...
\begin{equation}
	...
	\cos\TTalpha  = \dfrac{\left( \cos\TTalphaZero +\frac{\TTu }{\TTc } \right)}{\left(1+\frac{\TTu }{\TTc } \cos\TTalphaZero \right)}
\end{equation}
...
\begin{equation}
	...
	\cos\TTalphaZero  = \dfrac{\left( \cos\TTalpha  - \frac{\TTu }{\TTc } \right)}{\left(1 - \frac{\TTu }{\TTc } \cos\TTalpha \right)}
\end{equation}
...
For the case of a source, as its speed tends to the speed of light, $\TTu /\TTc $ tends to 1, and from equation \eqref{eq: cosine transform} we can see that, as a result of this, $\cos\TTalpha $ tends to 1.
...
\begin{figure}[H]
	\begin{subfigure}{.49\textwidth}
		...
		\begin{tikzpicture}[scale = 8]
			\draw[red,ultra thick,->] (0,0,0)--(0.235,-0.05,0) node[midway,anchor = north]{$\TTunderlinecZero $};
	...
	\begin{subfigure}{.49\textwidth}
		...
		\begin{tikzpicture}[scale = 8]
			...
			\node at (0.05,0.095,0) {$\TTalpha  $};
			\draw[cyan,ultra thick,->] (0,0,0)--(0.21,0.13,0) node[midway,anchor = north]{$\TTunderlinec $};
			\draw[ultra thick,->] (0,0,0)--(0,0.15,0)node[midway,anchor = east]{$\TTunderlineu $};
			\fill[black] (0,0,0) circle (0.7pt);
		\end{tikzpicture}
		\caption{moving frame}
		% \label{fig:sub1}
	\end{subfigure}
	\caption{\textbf{The Doppler effect.} Diagrams showing a source emitting light at a velocity $\TTunderlinecZero $ in the source's rest frame (left) and a second frame where the source is now moving at velocity $\TTunderlineu $ with the light now being emitted at an angle ${\TTalpha }$ giving the light's velocity as ${\TTunderlinec  }$. The different color of light in each frame is showing the effect on the lights frequency and hence color, due to the Doppler effect.}
	\label{fig: Doppler effect}
\end{figure}
%███████████

In the rest frame of the source, if it emits a wave at the speed of light in any direction, we have the length the wave has traveled from the source in the infinitesimal time ${\TTd \TTtZero }$ as being

\begin{equation}
	{\TTd \TTlambdaZero } = {\TTc }{\TTd \TTtZero }
\end{equation}

If we change to a frame where the source is moving at velocity ${\TTunderlineu  }$ in the z-direction, we have the wave emitted at an angle $\TTalpha $ from the z-axis, traveling a distance ${\TTc }{\TTd \TTt }$ in a time ${\TTd \TTt }$, with the source moving a distance $ \TTu  {\TTd \TTt } \cos\TTalpha  $ in the direction of this emitted wave.
...
\begin{equation}
	\begin{aligned}
		{\TTd \TTlambda } & \color{gray}= {\TTc }{\TTd \TTt }-{\TTu } \cos\TTalpha  {\TTd \TTt }   \\
		           & \color{gray}=  (1-\frac{\TTu }{\TTc }\cos\TTalpha )\TTc \TTd \TTt  \\
		           & = {\TTgamma } (1-\frac{\TTu }{\TTc }\cos\TTalpha )\TTc \TTd \TTtZero    \\
	\end{aligned}
\end{equation}

With ${\TTd \TTt } = {\TTgamma } {\TTd \TTtZero }$ being used in the last step, due to time dilation, from equation \eqref{eq: time dilation}.
The ratio of displacement the wave is taking up compared to in the \hyperlink{def-proper-frame}{rest frame} is

\begin{equation}
	\frac{\TTd \TTlambda }{\TTd \TTlambdaZero } = {\TTgamma } \left(1-\dfrac{\TTu }{\TTc } \cos\TTalpha  \right)
\end{equation}

Wavelength is inversely proportional to the frequency, and this bunching up at the front will proportionally decrease the wavelength, the frequency ratio $\TTnu /\TTnuZero $ is then

\begin{equation}
	\label{eq: Doppler Effect}
	\frac{\TTnu }{\TTnuZero } = \frac{\TTd \TTlambdaZero }{\TTd \TTlambda } = \frac{1}{ {\TTgamma } \left(1-\dfrac{\TTu }{\TTc } \cos\TTalpha  \right)}
\end{equation}
...
i.e. just in terms of $\TTv $ and not a sources velocity

\begin{equation}
	\frac{\TTnuPrime }{\TTnu } = {\TTgamma } \left(1-\dfrac{\TTv }{\TTc } \cos\TTalpha  \right)  = \frac{1}{ {\TTgamma } \left(1+\dfrac{\TTv }{\TTc } \cos\TTalphaPrime  \right)}
\end{equation}
...
\begin{figure}[H]
	...
	\end{subfigure}
	\caption{\textbf{Relativistic beaming.} Diagram showing, the position of an evenly spherically distributed light pulse in the sources rest frame (left) after propagating for a certain amount of time, with its current time ${\TTtZero }=0$, it is also shown in the moving frame using the Lorentz coordinate transform (middle) giving an ellipsoid with unsynchronized times, and also shown in the moving frame with each part of light pulse propagated forward or backwards in time to the origins time $\TTt =0$ (right). The black dot shows the current location of the source, with gray dot showing its position when the pulse was emitted. All the propagation of the light is in the direction that is outward from this point.}
	\label{fig Relativistic beaming}
\end{figure}
%███████████
...
\begin{equation}
	...
	\TTunderlinel  = \TTunderliner  - \TTunderlinec  \TTt
\end{equation}

Where $\TTunderliner $ is the lights positions from the Lorentz transformed rest frame coordinates at the Lorentz transformed time $\TTt $, with $\TTunderlinec $ being the velocity of light in the moving frame.
...
If the object is moving at velocity $\TTunderlineu $ you see it at a previous position $\TTunderlineu  {\TTT }_{\TTr \TTe \TTt }$, called its retarded position, where it was positioned at the retarded time ${\TTT }_{\TTr \TTe \TTt }$.
The time the object and the light took to propagate from this retarded position to where they currently are is the same, this time is $\TTTp =-{\TTT }_{\TTr \TTe \TTt }$.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[scale = 4.9] %[,tdplot_main_coords]
		...
		\draw[black, thick,->] (0,0.7,0)--(0.29,0.7,0) node[anchor = north]{${\TTy }$};
		\draw[black, thick,->] (0,0.7,0)--(0,1,0) node[anchor = west]{${\TTz }$};
		\draw[black, thick,->] (0,0.7,0)--(0,0.7,0.45) node[anchor = south east] {${\TTx }$};
		\draw[red, thick, dotted,->] (0,0,0)--(0.68,0.88,0) node[midway, right] {$\TTunderlinec  {\TTT }_{\TTp } $};
		\draw[red,->] (0,0.7,0)--(0.68,0.9,0) node[midway,above] {\text{$\TTunderlinel $} };
		\draw[blue, fill = blue] (0,0.7,0) circle (1pt);
		\draw[thick,blue,dotted,->] (0,0,0)--(0,0.66,0);
		\draw[blue, fill = blue] (0,0.7,0);
		\draw[blue, thick,->>] (0,0.7,0)--(0,0.8,0) node[left]{$\TTunderlineu $};
		\fill[red] (0.7,0.9,0) circle (0.4pt);
		\node[blue, right] at (0,0.4,0) {$\TTunderlineu  {\TTT }_{\TTp }$};
	\end{tikzpicture}
	\caption{\textbf{Retarded propagation.} Diagram showing, a source moving at velocity $\TTunderlineu  = (0,0,\TTu )$, the source is currently at the origin with time ${\TTt } = 0$, the source had emitted the light from the \protect\hyperlink{def-retarded-position}{retarded position} $-\TTunderlineu  {\TTT }_{\TTp }$ shown in gray (with ${\TTT }_{\TTp }$ being the propagation time of the light and source, from the emission), the light was then propagated along $\TTunderlinec  {\TTT }_{\TTp }$ shown by red dotted line, to reach position $\TTunderlinel $ at ${\TTt } = 0$.}
...
\begin{derivation}
	...
	\begin{equation}
		\TTunderlinec  \TTTp  = \TTunderlineu  \TTTp  + \TTunderlinel
	\end{equation}

	with $\TTunderlinel =(\TTlx ,\TTly ,\TTlz )$, writing out the vectors and rearranging to get the velocity vector for the light, gives

	\begin{equation}
		\label{eq: retarded light velocity vector}
		\TTunderlinec  = \frac{1}{\TTTp }
		\begin{pmatrix}
			\TTlx  \\
			\TTly  \\
			\TTlz  + \TTu  \TTTp
	...
	The magnitude of the light's displacement is equal to the distance the light travels in the corresponding propagation time $ \TTc  \TTTp  = \|\TTunderlinec  \TTTp \|$. The square of this gives

	\begin{equation}
		\begin{aligned}
			\left( \TTc  \TTTp \right)^2 & = \|\TTunderlinec  \TTTp \|^2                                     \\
			                      & = (\TTlx ^2 + \TTly ^2 + \TTlz ^2) + \TTu ^2 {\TTTp }^2 + 2\TTu  \TTTp  \TTlz ,
	...
	\begin{equation}
		{\TTTp }^2 - \left(2\TTgamma ^2\frac{\TTu }{\TTc ^2} \TTlz \right)\TTTp  - \frac{\TTgamma ^2}{\TTc ^2}(\TTlx ^2 + \TTly ^2 + \TTlz ^2) = 0,
	\end{equation}

	where $\TTgamma  = 1/ \sqrt{1 - \TTu ^2/\TTc ^2}$.
	Now taking the positive solution to this, which corresponds to the past time, and making use of the identity

	\begin{equation}
		\TTgamma ^2 = 1+\TTgamma ^2\frac{\TTu ^2}{\TTc ^2},
	\end{equation}
	...
	\begin{equation}
		\begin{aligned}
			\TTTp  & = \TTgamma ^2\frac{\TTu }{\TTc ^2} \TTlz  + \sqrt{\left(-\TTgamma ^2\frac{\TTu }{\TTc ^2} \TTlz  \right)^2+\frac{\TTgamma ^2}{\TTc ^2}(\TTlx ^2 + \TTly ^2+ \TTlz ^2)} \\
			    & = \TTgamma ^2\frac{\TTu }{\TTc ^2} \TTlz  + \frac{\TTgamma }{\TTc }\sqrt{\TTlx ^2+ \TTly ^2+\left(1+\TTgamma ^2\frac{\TTu ^2}{\TTc ^2}\right) \TTlz ^2}               \\
			    & = \TTgamma ^2\frac{\TTu }{\TTc ^2} \TTlz  + \frac{\TTgamma }{\TTc }\sqrt{\TTlx ^2 + \TTly ^2+\TTgamma ^2 \TTlz ^2}
		\end{aligned}
	\end{equation}
...
\begin{equation}
	...
	\begin{aligned}
		\TTLp  & \color{gray} = \TTc  \TTTp                                              \\
		      & \color{gray} = \TTgamma ^2 \frac{\TTu }{\TTc } \TTlz  + \TTgamma   \TTL             \\
		      & = \TTgamma  \left( 1 + \frac{\TTu }{\TTc } \frac{\TTgamma  \TTlz  }{\TTL } \right) \TTL
	\end{aligned}
\end{equation}

With $ \TTL =\sqrt{\TTlx ^2+\TTly ^2+\TTgamma ^2 \TTlz ^2}$.

Substituting this into equation \eqref{eq: retarded light velocity vector} we can find the light's velocity at $\TTunderlinel $ to be
...
\begin{equation}
	...
	\mhl{
		\TTunderlinec  =
		\frac{\TTc }{\TTgamma  \left( 1 + \frac{\TTu }{\TTc } \frac{\TTgamma  \TTlz  }{\TTL } \right)}
		\begin{pmatrix}
			\frac{\TTlx  }{\TTL } \\
			\frac{\TTly  }{\TTL } \\
			\TTgamma  \left( \frac{\TTgamma  \TTlz  }{\TTL } + \frac{\TTu }{\TTc } \right)
		\end{pmatrix}
	}
\end{equation}
...
\begin{equation}
	...
	\cos{\TTalpha } = \frac{\frac{\TTgamma  \TTlz  }{\TTL } + \frac{\TTu }{\TTc } }{1 + \frac{\TTu }{\TTc } \frac{\TTgamma  \TTlz  }{\TTL } }
\end{equation}

Earlier, in the derivation above, you may have noticed that we introduced the $\TTgamma $ term.
...
We have made use of the $\TTgamma $ term to simplify the equations like we do in special relativity, but we are not evoking special relativity here.
I just wanted to emphasize that the $\TTgamma $ term has a role to play for this too, and it has to be this way to allow special relativity to be self-consistent when swapping between frames.
...
Here, we will look at the propagation of light that had been emitted from a source at rest, then swap to a frame where the source is now moving, using the Lorentz transformation of the emitted light's current locations in the rest frame (from $\TTunderlinerZero $ to $\TTunderliner $), but remembering these Lorentz transformed times $\TTt $ are not synchronized with the sources time of zero.
To find this we will use the moving frame's velocity of light $\TTunderlinec $ to propagate back the light from where it is at $\TTt $ to where it is at the origin's time of zero.
...
\begin{figure}[H]
	\begin{subfigure}{.49\textwidth}
		...
		\begin{tikzpicture}[scale = 4.9] %[,tdplot_main_coords]
			%\usetikzlibrary{arrows.meta}
			\node[blue] at (0.06,-0.06,0) {$\TTsZero $};
			\draw[blue, thick,->] (0,0,0)--(0.4,0,0) node[anchor = north east]{${\TTyZero }$};
			\draw[blue, thick,->] (0,0,0)--(0,0.4,0) node[anchor = north east]{${\TTzZero }$};
			\draw[blue, thick,->] (0,0,0)--(0,0,0.55) node[anchor = south east] {${\TTxZero }$};
			...
			\fill[black] (0.71,0.48,0) circle (0.4pt) node[above left] {\text{$ \TTunderlinerZero $} };
		\end{tikzpicture}
		\caption{rest frame}
	...
	\begin{subfigure}{.49\textwidth}
		...
		\begin{tikzpicture}[scale = 4.9] %[,tdplot_main_coords]
			%\usetikzlibrary{arrows.meta}
			%node[anchor = north west]{\textcolor{black}{$\underline{P}'$} }; %$\underline{P}'$} };
			\node[gray] at (0.08,-0.06,0) {$\TTsret $};
			\draw[gray!40,fill = gray!40] (0,0,0) circle (1pt);
			\fill[red!40] (0.68*0.883,0.9*0.85,0) circle (0.4pt);
			\draw[black, thick,->] (0,0.7,0)--(0.29,0.7,0) node[anchor = north east]{ {\color{blue} ${\TTyZero }$},${\TTy }$};
			\draw[black, thick,->] (0,0.7,0)--(0,1,0) node[anchor = east]{ {\color{blue} ${\TTzZero }$},${\TTz }$};
			\draw[black, thick,->] (0,0.7,0)--(0,0.7,0.45) node[anchor = south east] { {\color{blue} ${\TTxZero }$},${\TTx }$};
			\draw[gray, dashed,->] (0,0,0)--(0.25,0,0) node[anchor = south east]{${\TTyZero }$};
			\draw[gray, dashed,->] (0,0,0)--(0,0.2,0) node[anchor = north east]{${\TTzZero }$};
			\draw[gray, dashed,->] (0,0,0)--(0,0,0.35) node[anchor = south east] {${\TTxZero }$};
			...
			\draw[black,->] (0,0.7,0)--(0.68,0.9,0) node[midway,above] {\text{$\TTunderliner $} };
			\draw[red,->] (0,0.7,0)--(0.68*0.85,0.9*0.85,0);
			\node[red] at (0.4,0.68,0) {\text{$\TTunderlinel $} };
			\draw[blue, fill = blue] (0,0.7,0) circle (1pt) node[anchor = north west]{$\TTs $};
			\draw[thick,blue,dotted,->] (0,0,0)--(0,0.66,0);
			\draw[blue, thick,->>] (0,0.35,0)--(0,0.4,0) node[left]{$\TTunderlineu $};
	...
	\end{subfigure}
	\caption{\textbf{Propagation of emitted light in the rest and moving frame.} Diagram showing, a source $\TTs $ in blue in its rest frame, Fig (a), with light that has propagated along the red dotted line, to the coordinate $\TTunderlinerZero $ shown in black, with the current time of ${\TTtZero } = 0$. Fig (b) shows the system in a frame with the source and its attached rest axis moving at velocity $\TTunderlineu  = (0,0,\TTu )$ in the z-direction, with the source currently at the origin in this frame with time ${\TTt } = 0$. When the light was emitted from the source, the source and attached rest axis were at the \protect\hyperlink{def-retarded-position}{retarded position} $\TTunderlinesret $ shown in gray, the emitted light was then propagated, along the red dotted line to reach $\TTunderlinel $ at ${\TTt } = 0$. With it reaching the coordinate $\TTunderliner $ which is the \protect\hyperlink{def-lorentz-transform}{Lorentz transform} of the rest frames coordinate at the Lorentz transformed time $\TTt =\TTgamma  \frac{\TTu }{\TTc ^2} \TTzZero $.}
	\label{fig: appendix Retarded field outward field transform}
\end{figure}
%███████████

Now to derive the lights position at time of zero in the moving frame, $\TTunderlinel $

%█████████████████
\begin{derivation}

	As visualized in the above in figure (\ref{fig: appendix Retarded field outward field transform}).
	If we have a source in its \hyperlink{def-proper-frame}{rest frame}, that had emitted light from the origin and propagated to its current position $\TTunderlinerZero $ at time ${\TTtZero } = 0$.
	Then using the coordinate transform from equation \eqref{eq: Lorentz transformation} on position $\TTunderlinerZero =\TTunderlinelZero =(\TTlZerox ,\TTlZeroy ,\TTlZeroz )$ with time ${\TTtZero } = 0$, we get
	...
	\begin{equation}
		...
		\begin{aligned}
			...
			\begin{pmatrix}
				...
				{\TTgamma } \TTlZeroz  \\
			\end{pmatrix},                                                                         \\
			\text{at time: }\quad& {\TTt }=-{\TTgamma } \frac{ {\TTv } }{ {\TTc }^2}{\TTlZeroz  } = {\TTgamma } \frac{\TTu }{ {\TTc }^2}{\TTlZeroz  } \\
		\end{aligned}
	\end{equation}
	...
	\begin{equation}
		...
		% 	\dfrac{u}{c} + \frac{z}{R}   \\
		% \end{pmatrix}
		\dfrac{\TTc }{\TTgamma  \left(1 + \dfrac{\TTu }{\TTc } \dfrac{\TTlZeroz  }{\|\TTunderlinelZero \|} \right)}
		\begin{pmatrix}
			\frac{\TTlZerox  }{\|\TTunderlinelZero \|}                                      \\
			\frac{\TTlZeroy  }{\|\TTunderlinelZero \|}                                      \\
			\TTgamma  \left( \frac{\TTlZeroz  }{\|\TTunderlinelZero \|} + \frac{\TTu }{\TTc }  \right) \\
		\end{pmatrix}
	\end{equation}
	...
	Now we have enough to find position $\TTunderlinel $ where the light is positioned when its time is synchronized with the source.
	We could use the triangle formed in figure (\ref{fig: primed frame 1}), which gives $\TTunderlinel  = \TTunderlinec  \TTTp  -\TTunderlineu  \TTTp $ like we did in the previous section.
	...
	\begin{equation}
		\begin{aligned}
			...
			% \\
			 & \color{gray} = \TTunderliner  - \TTunderlinec  \TTt
			\\
			 & \color{gray} =
			\begin{pmatrix}
				\TTlZerox           \\
			...
			\end{pmatrix}
			-
			\frac{\frac{\TTu }{\TTc } {\TTlZeroz  } }{1 + \frac{\TTu }{\TTc } \frac{\TTlZeroz  }{\|\TTunderlinelZero \|} }
			\begin{pmatrix}
				\frac{\TTlZerox  }{\|\TTunderlinelZero \|}                                      \\
				\frac{\TTlZeroy  }{\|\TTunderlinelZero \|}                                      \\
				\TTgamma  \left( \frac{\TTlZeroz  }{\|\TTunderlinelZero \|} + \frac{\TTu }{\TTc }  \right) \\
			\end{pmatrix} \\
			 & \color{gray} =
			\frac{1}{1 + \frac{\TTu }{\TTc } \frac{\TTlZeroz  }{\|\TTunderlinelZero \|} }
			\begin{pmatrix}
				\TTlZerox  + \frac{\TTu }{\TTc } \frac{\TTlZeroz  }{\|\TTunderlinelZero \|} \TTlZerox  - \frac{\TTu }{\TTc } {\TTlZeroz  } \frac{\TTlZerox  }{\|\TTunderlinelZero \|}                                                         \\
				\TTlZeroy  + \frac{\TTu }{\TTc } \frac{\TTlZeroz  }{\|\TTunderlinelZero \|} \TTlZeroy  - \frac{\TTu }{\TTc } {\TTlZeroz  } \frac{\TTlZeroy  }{\|\TTunderlinelZero \|}                                                         \\
				{\TTgamma } \TTlZeroz  + {\TTgamma } \frac{\TTu }{\TTc } \frac{\TTlZeroz  }{\|\TTunderlinelZero \|} \TTlZeroz  - \frac{\TTu }{\TTc } {\TTlZeroz  }  \TTgamma  \left( \frac{\TTlZeroz  }{\|\TTunderlinelZero \|} + \frac{\TTu }{\TTc }  \right) \\
			\end{pmatrix}
		\end{aligned}
	\end{equation}
...
\begin{equation}
	...
	\mhl{
		\begin{aligned}
			...
			% 	\frac{1}{ {\gamma} } z \\
			% \end{pmatrix}
			\dfrac{1}{1 + \dfrac{\TTu }{\TTc } \dfrac{\TTlZeroz  }{\|\TTunderlinelZero \|} }
			\begin{pmatrix}
				\TTlZerox                     \\
				\TTlZeroy                     \\
				\frac{1}{ {\TTgamma } } \TTlZeroz  \\
			\end{pmatrix}
			\\
			 & \text{at time ${\TTt }=0$}
		\end{aligned}
...
After this we looked at emitted light at time $\TTtZero =0$ in the sources rest frame, and how to transform it to a frame where the source is moving, to a time $\TTt =0$.
...
Equation \eqref{eq: time synced transform} was used to transform each part of the propagating light to where it is at time $\TTt =0$ (the time that it is synchronized with the source at the origin).
...
So if we define a sphere with radius $\TTr $ we can find an infinitesimal portion of the surface of the sphere at angles ${\TTtheta }$ and ${\TTphi }$ as shown on this figure:
...
\begin{figure}[H]
	...
	\scalebox{1}{%0.6}{
		...
		\begin{tikzpicture}[tdplot_main_coords]
			...
			\begin{pgfonlayer}{background}
				\draw[thick,-latex] (0,0,0)--(1.2*\rvec,0,0) node[pos = 1.1]{${\TTx }$};
				\draw[thick,-latex] (0,0,0)--(0,1.2*\rvec,0) node[anchor = south east]{${\TTy }$};
				\draw[thick,-latex] (0,0,0)--(0,0,1.2*\rvec) node[pos = 1.05]{${\TTz }$};
			\end{pgfonlayer}
			%
			%Help Lines
			\begin{pgfonlayer}{foreground}
				%Up
				\draw[ultra thick, red,->] (O)--(A) node[pos = 0.6, left] {${\TTunderliner  }$};
				\draw (O)--(B);
				\draw (O)--(C);
				\draw[dashed] (O)--(D);
				%Down
			...
			\begin{pgfonlayer}{foreground}
				...
				%
				\tdplotdrawarc[tdplot_main_coords]{(O)}{\rvec + \drvec}{\phivec}{\phivec + \dphivec}{below, rotate = 13}{$\TTr \TTmathrmd \TTphi $}
			\end{pgfonlayer}
			%
			%Angles
			\begin{pgfonlayer}{foreground}
				...
				\node at (0.8,0.3,0) {$\TTphi $};
				\tdplotdrawarc[-stealth]{(O)}{1.5}{\phivec}{\phivec + \dphivec}{}{}
				\node at (1.4,1.9,0) {$\TTmathrmd \TTphi $};
				\tdplotsetthetaplanecoords{\phivec}
				%Theta
				\tdplotdrawarc[tdplot_rotated_coords,-stealth]{(0,0,0)}{1.5}{0}{\thetavec}{}{}
				\node at (0,0.15,0.8) {$\TTtheta $};
				\tdplotdrawarc[tdplot_rotated_coords,-stealth]{(0,0,0)}{1.5}{\thetavec}{\thetavec + \dthetavec}{anchor = south west}{$\TTmathrmd \TTtheta $}
			\end{pgfonlayer}
			%
			%Differential Volume
			%
			...
			\begin{pgfonlayer}{foreground}
				...
				%
				\tdplotsetthetaplanecoords{\phivec + \dphivec}
				\tdplotdrawarc[tdplot_rotated_coords, thick]{(O)}{\rvec + \drvec}{\thetavec}{\dthetavec + \thetavec}{above right}{$\TTr \TTmathrmd \TTtheta $}
				%
				\tdplotsetrotatedcoords{55}{-50.4313}{-6.4086}
				...
				\node[anchor=center, rotate=14, below] at (1.5,2.78,1.8) {$\TTr \TTmathrmd \TTphi  \sin\TTtheta $};
			\end{pgfonlayer}
			%
			%Fill Color
	...
	\caption{\textbf{Differential surface element on a sphere} This shows an infinitesimal surface element on a sphere of radius $\TTr $ that spans from $\TTtheta $ to $\TTtheta  + \TTd \TTtheta $ and $\TTphi $ to $\TTphi  + \TTd \TTphi $ in spherical polar coordinates, the infinitesimal surface element is equivalent to a rectangle with sides $\TTr  \TTd \TTtheta $ and ${\TTr }\sin\TTtheta  {\TTd \TTphi }$, giving the infinitesimal surface area of $\TTr ^2 \sin\TTtheta  \,\TTmathrmd \TTtheta  \,\TTmathrmd \TTphi $.}
\end{figure}
%███████████

The surface element of a sphere at a constant radius ${\TTr }$, spanning from $\TTtheta $ to $\TTtheta  + \TTd \TTtheta $ and $\TTphi $ to $\TTphi  + \TTd \TTphi $, is equivalent to a rectangle with sides ${\TTr }{\TTd \TTr }$ and ${\TTr }\sin\TTtheta  {\TTd \TTphi }$ giving the surface area as

\begin{equation}
	\TTmathrmd \TTS  =  \TTr ^2 \sin\TTtheta  \,\TTmathrmd \TTtheta  \,\TTmathrmd \TTphi
...
This surface element's size depends on the angle from the z-axis and the distance from the origin, but if we want a quantity that only depends on the angle, we can divide the surface element by $\TTr ^2$, this gives what is called the solid angle.
The solid angle element $\TTd \TTOmega $ is a measure of the amount of the field of view, from the origin, that the surface element covers. For the differential surface element shown in the figure we have the differential solid angle

\begin{equation}
	\label{eq: solid angle}
	\TTmathrmd \TTOmega  = \frac{\TTmathrmd \TTS }{\TTr ^2} = \sin\TTtheta  \,\TTmathrmd \TTtheta  \,\TTmathrmd \TTphi
\end{equation}

If we were to integrate this over the whole sphere $ \int_{0}^{2\TTpi } \int_{0}^{\TTpi } \, \sin \TTphi  \, \TTd \TTtheta  \, \TTd \TTphi $, we would get $4\TTpi $, which is when the view from the origin is fully covered by the surface.
Therefore, the fraction of the view from the origin that is covered by a solid angle $\TTOmega $ is $\TTOmega / 4\TTpi $.
...
\begin{derivation}
	...
	If we have a source at rest emitting light in all directions we can say at certain angles given by the spherical polar coordinates $\TTtheta  = \TTalphaZero $ and $\TTphi = \TTvarphiZero $ we have a quantity $\TTd \TTQ $ of the emitted light's wavefront that is encompassed in the infinitesimal solid angle

	\begin{equation}
		\TTd \TTOmegaZero  = \sin{\TTalphaZero } \TTd \TTalphaZero  \TTd \TTvarphiZero ,
	\end{equation}

	this is the same amount of the light's wavefront that is encompassed by the coinciding solid angle in the moving frame

	\begin{equation}
		\TTd \TTOmega  = \sin{\TTalpha } \TTd \TTalpha  \TTd \TTvarphi .
	\end{equation}

	We can calculate this element by differentiating both sides of equation
	\eqref{eq: cosine transform} with respect to $\TTalphaZero $ \cite{hogg1997special}, which after doing so, gives

	\begin{equation}
		\sin{\TTalpha } \TTd \TTalpha  = \dfrac{1}{ {\TTgamma }^2\left(1 + \dfrac{\TTu }{\TTc }\cos{\TTalphaZero }\right)^2} \sin{\TTalphaZero } \TTd \TTalphaZero
	\end{equation}

	now using this and ${\TTd \TTvarphi } = {\TTd \TTvarphiZero }$ (as the angle ${\TTvarphiZero }$ is always perpendicular to the motion of the particle and unaffected by the \hyperlink{def-transform}{transformation}) we have the solid angle in the moving frame given as

	\begin{equation}
		\TTd \TTOmega  = \dfrac{1}{ {\TTgamma }^2\left(1 + \dfrac{\TTu }{\TTc }\cos{\TTalphaZero }\right)^2} \sin{\TTalphaZero } \TTd \TTalphaZero  \TTd \TTvarphiZero .
	\end{equation}
	...
	If there is the same quantity of light $\TTd \TTQ $ in the solid angle element in both frames, then the density of light in the solid angle in each frame is $\TTd \TTQ /\TTd \TTOmegaZero $ and $\TTd \TTQ /\TTd \TTOmega $ respectively, giving the ratio of the density in the solid angle element in the moving frame relative to the rest frame as

	\begin{equation}
		\TTDsigma  = \frac{\TTd \TTOmegaZero }{\TTd \TTOmega } = {\TTgamma }^2\left(1 + \dfrac{\TTu }{\TTc }\cos{\TTalphaZero }\right)^2.
	\end{equation}

	By substituting equation \eqref{eq: cos alpha prime in terms of l} into equation \eqref{eq: inverse cosine transform} we get $\cos\TTalphaZero =\frac{\TTgamma  \TTlz  }{\TTL }$, it allows us to get this in terms of the light's position in its moving frame

	\begin{equation}
		\TTDsigma  = {\TTgamma }^2\left(1 + \dfrac{\TTu }{\TTc }\frac{\TTgamma  \TTlz  }{\TTL }\right)^2.
	\end{equation}

	with $\TTL  = \sqrt{\TTlx ^2 + \TTly ^2 + \TTgamma ^2 \TTlz ^2}$.

	$\TTDsigma $ is the density of light in a solid angle element for a moving source relative to the density of light in the corresponding solid angle element in the rest frame of the source.

\end{derivation}
%███████████

At position $(\TTlx ,\TTly ,\TTlz )$, light has propagated a distance $\TTLp $ from where it was emitted, given in equation \eqref{eq: propagation distance},
Its surface area element grows by a factor of the $\TTLp ^2$ and the light's density in a surface elements area dissipates by the inverse of this

\begin{equation}
	\frac{1}{\TTLp ^2} = \frac{1}{\TTgamma ^2 \left( 1 + \frac{\TTu }{\TTc } \frac{\TTgamma  \TTlz }{\TTL } \right)^2 \TTL ^2}
\end{equation}

Multiplying this by the density of light in the solid angle element relative the rest source, $\TTDsigma /\TTLp ^2$, we get the density of light in the surface element given as

\begin{equation}
	\label{eq: denisty of light in surface element}
	\mhl{
		\begin{aligned}
			\TTDsigma  = \frac{\TTksigma  }{\TTL ^2}                                                     \\
			\color{gray} \text{with } \TTL  & \color{gray}= \sqrt{\TTlx ^2 + \TTly ^2 + \TTgamma ^2 \TTlz ^2}
		\end{aligned}
	}
\end{equation}

Where $\TTksigma $ is the proportionality constant.

For a source at rest $\TTu =0$ we have $\TTgamma  = 1$ and this then just gives the inverse square of the distance from the rest source, the inverse square law of the distance from the source

\begin{equation}
	\TTDsigma  = \frac{\TTksigma  }{\TTlx ^2 + \TTly ^2 + \TTlz ^2}
\end{equation}

This is what we will get when setting $\TTu =0$ for all the properties of the light field that we will look at in each section.

$\frac{1}{\TTL ^2}$ will show up several times for a moving source in this chapter, and is plotted for a moving source and a source at rest, below.
...
\begin{figure}[H]
	...
	\end{subfigure}
	\caption{\textbf{Surface element's light density}. This Figure shows the density of light in a surface element at $\TTDsigma $, any point in the y-z plane, the value of this goes to infinity as we get closer to the source, so for us to be able to color the graph nicely we needed to remove the center, shown by the black circle in the center.}
...
\begin{equation}
	\label{eq: denisty of light parallel to propagation}
	\TTDc  = {\TTgamma }\left(1 + \dfrac{\TTu }{\TTc }\frac{\TTgamma  \TTlz }{\TTL }\right) \TTkc
\end{equation}

Where $\TTkc $ is the constant density of light in the direction of the light's propagation for a stationary source.
...
In this chapter we will remain in the same frame, and looking at a source moving at speed $\TTu $.
The speed will be set to zero, $\TTu =0$, to look at the case of a source at rest.
But we will remain in the same frame, its just that the sources velocity is different.
We will therefore use the same notation for coordinates throughout this chapter, $(\TTx ,\TTy ,\TTz )$.
...
\begin{equation}
	...
	\TTunderlinel  = (\TTlx ,\TTly ,\TTlz ) = (\TTx ,\TTy ,\TTz )
\end{equation}
...
We have the density of light in the direction of its propagation $\TTDc $ and in the plane perpendicular to it $\TTDsigma $, given in equations \eqref{eq: denisty of light in surface element} and \eqref{eq: denisty of light parallel to propagation}.

Giving the volume density to be

\begin{equation}
	\begin{aligned}
		\TTD  & \color{gray} = \TTDsigma  \TTDc                                                          \\
		  & = {\TTgamma }\left(1 + \dfrac{\TTu }{\TTc }\dfrac{\TTgamma  \TTz }{\TTL }\right) \frac{\TTksigma  \TTkc  }{\TTL ^2}
	\end{aligned}
\end{equation}

This density of light for a moving source at any point can be seen in the following diagram, with the left figure showing the volume density of light emitted from a source at rest, when $\TTu =0$, showing the inverse square law.
...
Consider a rapidly rotating rigid disk. The circumference moves parallel to the motion and should contract ($\TTC  < 2\TTpi  \TTR $). The radius moves perpendicular to motion and stays the same.
The Paradox: Euclidean geometry ($\TTC  = 2\TTpi  \TTR $) fails on the disk.
