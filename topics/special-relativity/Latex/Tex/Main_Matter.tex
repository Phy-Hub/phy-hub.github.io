...
The time the object and the light took to propagate from the objects retarded position to where they currently are is the same, $T_{p}=-{T}_{ret}$.
We can use the following figure to find the retarded position, and work out the distance $R_{p}=\|\mathbf{c} {T}_{p}\|$ it has traveled from the retarded position in terms of the positions coordinates and the objects velocity.
...
*** put in $t'_p = \gamma t_p$ i believe
...
*** This \hyperlink{def-flux}{flux} transform is the change in \hyperlink{def-flux}{flux} in the emitted direction from the source only not what another particle would feel
