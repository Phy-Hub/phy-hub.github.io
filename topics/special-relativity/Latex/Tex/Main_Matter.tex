...
\begin{figure}[H]
	...
	\caption{\textbf{Light clock on a truck.} A diagram, showing a clock consisting of light being reflected between the floor and roof of a truck to keep time, in the trucks frame (top), and in the road's frame with the truck moving (bottom).}
	\label{fig: truck clock}
\end{figure}
%███████████

Given that the distance traveled by the light in the frame with the truck moving is longer, yet experimentally we know that the speed of light is the same for both reference frames, as shown in previous sections, to keep the speed of light invariant: the clock itself must tick slower in moving objects,

There was also the possibility that the height of the moving truck could be contracted, to give the same path length for light so that the ticks happen at the same rate, but this leads to paradoxes as shown in Figure (\ref{fig: width contraction}).
...
An observer on the road and in the truck experience time at different rates. This is not just an illusion, it is physical reality.
...
Each truck would see its light moving vertically up and down with the other truck's light moving diagonally.
Hence, each truck perceives the other truck's clock ticking slower.
An observer in the road's reference frame would see both trucks move at the same speed and, hence, the passage of time between ticks as being the same for both trucks.
The slowing of time of moving objects is the same regardless of its relative direction of motion, only the relative speed changes the path distance of the ticking light.
...
The time between the light being emitted and absorbed is the \hyperlink{def-time-dilation}{dilated time}, that was described in section (\ref{Subsect: Time Dilation}).
...
We can work out the length of the upward path from the \hyperlink{def-time-dilation}{time dilation} section (\ref{Subsect: Time Dilation}), and this is the length the path needs to be in the horizontal directions as well,



the paths can only have this length if the truck's length is contracted when moving,

*** explain why is length contraction is none contradictory/paradoxical while we have previously seen perpendicular contraction to be so

*** could say that an option is to look at the posibility of length contraction,
*** or just state that when it is worked out where the light (which is moving at a constant speed) must be reflected by the walls to reach the center simultaneousthe point at which the horizontal light is reflected must be such that the lorry is observed to be contracted
*** and meantion that this may seem like a non possible option as the width contract leads to contradictions, but it turns out this length contract does not suffer from the same paradox


the exact amount of contraction can be worked geometrically, and it turns out that the ratio of the increase in the amount of time that passes before the light is reabsorbed is inversely proportional to the amount the truck is contracted in the direction of its movement.
...
\begin{figure}[H]
	...
	... *** The bottom image could be confusing to some. As the vertical and horizontal path lengths should be equivalent for simultaneity, though the diagram appears to show that the vertical path is longer... make diagram clearer}
...
So if we have a source in its rest frame that continually emits pulses of light with a constant time between each pulse, we get what is shown in Figure (\ref{fig: multiple emitting pulses rest frame}).
...
We also have the dilated time between pulses in this moving frame, so using these and the beaming effect from the previous section we get the light's vector field as shown in Figure (\ref{fig: multiple emitting pulses moving frame}).
...
which will be needed when looking at aberration.

So lets start with how to describe a vector in terms of the Spherical Polar Coordinates, shown in the following diagram
...
\begin{figure}[H]
	...
	\end{tikzpicture}
	\caption{\textbf{Spherical polar coordinates.} Diagram of an $x,y,z$-axis showing a position vector $\mathbf{r}$ with its associated spherical polar coordinates $(r,\theta,\phi)$, where $\theta$ is the angle from the $z$-axis, $\phi$ is the angle rotated around $z$-axis starting from the $x$-axis, and $r$ is the magnitude .}
\end{figure}
%███████████
...
\begin{figure}[H]
	...
	\scalebox{1}{%0.6}{
		\begin{tikzpicture}[tdplot_main_coords]
			...
			\begin{pgfonlayer}{foreground}
				%Phi
				\tdplotdrawarc[-stealth]{(O)}{1.5}{0}{\phivec}{anchor = north}{$\phi$}
	...
	\caption{\textbf{Differential surface element on a sphere} This shows a surface element on a sphere of radius $r$ that spans from $\theta$ to $\theta + d\theta$ and $\phi$ to $\phi + d\phi$ in spherical polar coordinates, the surface element has sides $r d\theta$ and ${r}\sin\theta {d\phi}$, giving the infinitesimal surface area of $r^2 \sin\theta \,\mathrm{d}\theta \,\mathrm{d}\phi$.}
\end{figure}
%███████████

The surface element at a constant radius ${r}$, spanning from $\theta$ to $\theta + d\theta$ and $\phi$ to $\phi + d\phi$ on a spherical surface has sides ${r}{dr}$ and ${r}\sin\theta {d\phi}$ giving the surface area as
...
We will also introduce the idea of a solid angle $\Omega$ which is a measure of the amount of the field of view from some particular point that a given object covers. For the differential surface element from the figure we have the differential solid angle
...
This is the relativistic aberration formula \cite{einstein1905electrodynamics}, it shows how the light's propagation direction transforms as shown in Figure (\ref{fig: aberrated emitted light}).
...
Let us look now at a source continuously emitting light in all directions in its rest frame.
...
The magnitude of the retarded field displacement is equal to the distance the field, propagating at the speed of light, travels in the corresponding retarded time. This gives
\begin{equation}
	\begin{split}
...
Now to see the rate of change of the direction that the light propagates, by taking the partial derivative of $\mathbf{\hat{c}}$ with respect to each coordinate
...
% \begin{equation}
% 	\mathbf{\hat{c}} \cdot \frac{\partial \mathbf{\hat{c}}}{\partial x} = x (1 - \frac{x^2}{M^2} + \frac{u}{c}\frac{\gamma_u z}{M}) + y (- \frac{x}{M} \frac{y}{M}) + (\gamma^2 z + \gamma \frac{u}{c} M)(- \frac{x}{M} \frac{z}{M})=0
% \end{equation}

% \begin{equation}
% 	\mathbf{\hat{c}} \cdot \frac{\partial \mathbf{\hat{c}}}{\partial z} = x (( - \frac{u}{c}  - \frac{ \gamma_u z }{M} ) \frac{x}{M}) + y (( - \frac{u}{c}  - \frac{\gamma_u z }{M} ) \frac{y}{M}) + (\gamma^2 z + \gamma \frac{u}{c} M)(\frac{1}{\gamma_u}\frac{ x^2 + y^2 }{M^2})=0
% \end{equation}

%███████████████████████████████████████████████████████████████████
\subsection{The rate of change or potential}\label{subsect: The rate of change or potential}
...
%███████████████████████████████████████████████████████████████████
%███████████████████████████████████████████████████████████████████
\section{Flux/flowrate  /  radial/angular concentration/density of light from Point Source} \label{sect: Flux from Point Source}
...
% \eqref{eq: velocity transform for light} in Cartesian coordinates and particles speed in the \hyperlink{def-Primed-Frame}{primed frame} with ${v} =-{u}$ is
...
% 		\\
% 		 & = \left( \mathbf{c}{'}-\mathbf{u} \right)t{'}_{p}
% 		\\
...
% so lets find the directions of the propagation at time using for time at zero

% \textbf{General Equations for Light Propagation Direction:}
...
% \textbf{General Infintesmal Change in all Spatial Coordinates:}
...
% \textbf{Infintesmal Change in $z$-direction:}
...
% substituting in the denominator times and removing negligible terms and, we have
...
% \textbf{Infintesmal Change in $x$-direction:}
...
% \textbf{Infintesmal Change in time:}
