...
To grasp special relativity, we will first look at the classical understanding of the world.
...
With the help of the \hyperlink{def-length-contraction}{length contraction} section (\ref{sect: Length Contraction}), Consider a truck with a spherical mirrored container, with a central bulb emitting light in all directions.
...
%███████████████████████████████████████████████████████████████████
%███████████████████████████████████████████████████████████████████
\chapter{Position, Time and Velocity} %The Math Begins:
...
This will give us something to compare special relativity to later, to help with our intuition.
...
How the positions and times of events are transformed between two inertial frames to allow the speed of light to stay constant, is what all the other concepts of special relativity are built upon.
...
First by deriving the time dilation, and length contraction in order to find the full transform of the space and time coordinates between two inertial frames.
...
After this, we will look at what the transform for infinitesimal lengths and changes in time looks like and use this to derive how velocities and accelerations change when swapping between inertial frames.
By the end of this chapter you should understand the coordinate transforms conceptually and mathematically when swapping between two frames, and also how to derive the velocity and acceleration transforms.
...
Before special relativity, it was assumed that distances between two positions were the same for all \hyperlink{def-observer}{observer}s. That is, the first \hyperlink{def-observer}{observer} would see a stick and measure it to be the same length as another \hyperlink{def-observer}{observer} moving relative to them would measure it to be.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[scale = 3]%,tdplot_main_coords]
		...
		%
	\end{tikzpicture}
	\caption{\textbf{An event in initial frame with a primed frame's axis moving relative to its axis}.	This shows an initial frame with its associated x,y,z-coordinate system at rest in its frame, with origin ${O}$. There is also an x',y',z'-coordinate system with origin ${O'}$ associated with a "primed" frame, at position ${\vec{OO'}}$ in the initial frame. It is moving at speed ${v}$ in the z-direction relative to the initial frame. An event ${E}$ in this initial frame is described by a set of spatial and time coordinates ${\vec{OE}} = ({x},{y},{z})$ and ${t}$. We choose the time to be zero in both frames when both axes overlapped, so that ${t}$ is also the time from when they overlapped. We also show ${\vec{O'E}}$ which represents the vector from the primed origin to the event in the initial frame.}
	\label{fig: simple Galilean transform derivation}
\end{figure}
%███████████

As shown in the diagram we have an event in this frame located at

\begin{equation}
	{\vec{OE}} = ({x},{y},{z})
\end{equation}

at time ${t}$ from when both axes overlapped and a prime frame's axis origin ${O{'}}$ moving at relative velocity ${v}$ in the z-direction so that in the initial frame we have
...
\begin{figure}[H]
	\begin{subfigure}{.49\textwidth}
		...
		\begin{tikzpicture}[scale = 0.65]
			...
			\draw[red, dashed, thick arrows] (6, 6.15)--(0, 6.15) node[midway, above] {${l}$};
	...
	\begin{subfigure}{.49\textwidth}
		...
		\begin{tikzpicture}[scale = 0.65]
			...
			% Draw the dashed arrowed lines with custom thick arrowheads
			\draw[red, dashed, thick arrows] (A3)--($(B2)!0.02!(A3)$) node[midway, below] {${l^{'}}$};
			\draw[red, dashed, thick arrows] (B2)--(A1) node[midway, below] {${l^{'}}$};
			...
			\draw[<->, thick] (-1, 0)--(-1, 6) node[midway, left] {${D}$};

		\end{tikzpicture}
		\caption{Moving frame}
	\end{subfigure}
	\caption{\textbf{Light clock showing time dilation.} The light clock keeps time by counting each time the light makes a return journey when bouncing between two mirrors with length ${L}$ between them, the diagram shows the mirror's rest frame (left) and a frame with them moving at speed ${u}$ (right). In this second frame, there is a distance ${D}$ between where the light was reflected by the left mirror and where it returned to it. For this, the light had to travel a diagonal distance of $2{l^{'}}$ to return.}
...
This is the hypotenuse of a triangle formed by ${D}$ and $2{L}$, so we have the square of the hypotenuse equal to the square of the other two lengths:
...
In the moving frame, the pulse sent forward has a distance
...
This means that the distances in the direction of movement of an object are contracted in its direction of movement compared to the rest frame of the object, that is
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[scale = 3]%,tdplot_main_coords]
		...
		%
	\end{tikzpicture}
	\caption{\textbf{An event in initial frame with a primed frame's axis moving relative to it.} This is a diagram of an initial frame with its associated $x,y,z$-coordinate system at rest in its frame, with origin ${O}$. There is also an $x',y',z'$-coordinate system with origin ${O'}$ associated with a "primed" frame, at position ${\vec{OO'}}$ in the initial frame. It is moving at speed ${v}$ in the z-direction relative to the initial frame. An event ${E}$ in the initial frame is described by a set of spatial and time coordinates ${\vec{OE}} = ({x},{y},{z})$ and ${t}$. We choose the time to be zero in both frames when both axes overlapped, so that ${t}$ is also the time from when they overlapped. We also show ${\vec{O'E}}$ which represents the vector from the primed origin to the event in the initial frame.}
...
then in the primed frame, we will the event described by the primed coordinates that we want to transform into, as
...
with ${\gamma}^{-2} = 1-{v}^2/{c}^2$ leading to the prime frames time for the event given in initial frames coordinates as

\begin{equation}
	{t{'}} = {\gamma} \left( t-\dfrac{{v}{z}}{{c}^2} \right)
\end{equation}

again if we want the inverse transform from prime to initial frame we can swap the prime and initial frame coordinates and put a minus in front of the frame velocity.
...
To get the velocity transform equations we will first need the equations to transform the displacement between two positions and times, and take them in the limit as the changes go to zero.
That is when we have infintesmal changes of coordinates.
...
rearranging and replacing the difference of the each the first and second coordinates, ${\Delta}{\vec{R}} = ({\Delta}x,{\Delta}y,{\Delta}z) = (x_2-x_1,y_2-y_1,z_2-z_1)$, giving the displacement transform as
...
so when we make an infintesmal small change in coordinate or time in a frame, we know what the corresponding change is in a primed frame.
...
can you derive how paths of a particles transform into a second frame
