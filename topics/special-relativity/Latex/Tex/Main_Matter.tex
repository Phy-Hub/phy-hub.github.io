...
\begin{figure}[H]
	\begin{subfigure}{.49\textwidth}
		...
		\begin{tikzpicture}[scale = 0.65]
			...
			\draw[red, dashed, thick arrows] (6, 6.15)--(0, 6.15);

			% Draw the distance labels
			\draw[<->, thick] (0, 4.7)--(6, 4.7) node[midway, above] {${W}$};
	...
	\begin{subfigure}{.49\textwidth}
		...
		\begin{tikzpicture}[scale = 0.65]
			...
			% Draw the dashed arrowed lines with custom thick arrowheads
			\draw[red, dashed, thick arrows] (A3)--($(B2)!0.02!(A3)$) node[midway, below, sloped] {${D}$};
			\draw[red, dashed, thick arrows] (B2)--(A1) node[midway, below, sloped] {${D}$};

			% Draw the velocity arrow
			\draw[black, thick,->, line width = 1.5pt] (3, 6)-- +  + (0, 0.5) node[midway, right] {${u}$};

			% Draw the distance labels
			\draw[<->, thick] (0,-1.3)--(6,-1.3) node[midway, above] {${W}$};
			\draw[<->, thick] (-1, 0)--(-1, 6) node[midway, left] {${L}$};

		\end{tikzpicture}
		\caption{Moving frame}
	\end{subfigure}
	\caption{\textbf{Light clock showing time dilation.} The light clock keeps time by counting each time the light makes a return journey when bouncing between two mirrors with distance ${W}$ between them, the diagram shows the mirror's rest frame (left) and a frame with them moving at speed ${u}$ (right). In this second frame, there is a distance ${L}$ between where the light was reflected by the left mirror and where it returned to it. For this the light had to travel a diagonal distance of $2{D}$ to return.}
...
This is the hypotenuse of a triangle formed by ${L}$ and $2{W}$, so we have the square of the hypotenuse equal to the square of the other two lengths:
...
This is used throughout special relativity to simply equations.
...
\begin{figure}[H]
	...
	\label{fig: length contraction math *** again remove the left mirror to simplify, also could add initial and primed frame axis moving relative to eachother to the side of each image, remove $<s>$}
\end{figure}
%███████████
...
\begin{equation}
	{c}{t{'}} = {c}{\gamma} {t} = {\gamma} {D} = {\gamma}^2 {D{'}}
\end{equation}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[scale = 3]%,tdplot_main_coords]
		...
		%
	\end{tikzpicture}
	\caption{\textbf{An event in initial frame with a primed frame's axis moving relative to its axis}.	This shows an initial frame with its associated x,y,z-coordinate system at rest in its frame, with origin ${O}$. There is also an x',y',z'-coordinate system with origin ${O'}$ associated with a “primed” frame, at position ${\vec{OO'}}$ in the initial frame. It is moving at speed ${v}$ in the z-direction relative to the initial frame. An event ${E}$ in this initial frame is described by a set of spatial and time coordinates ${\vec{OE}} = ({x},{y},{z})$ and ${t}$. We choose the time to be zero in both frames when both axes overlapped, so that ${t}$ is also the time from when they overlapped. We also show ${\vec{O'E}}$ which represents the vector from the primed origin to the event in the initial frame. *** rewrite for Relaivistic version}
	\label{fig: simple Lorentz transform derivation}
\end{figure}
%███████████

If we are in an intial frame with origin ${O}$ at position $(0,0,0)$ and a primed frame moving in the z-direction at velocity ${v}$ such that the position of the primed origin in this frame is

\begin{equation}
	\vec{OO{'}} = (0,0,{v}{t})
\end{equation}

at time ${t}$ from when the origins overlapped, if there is an \hyperlink{def-event}{event} (say light or another object reaching a mirror that is moving with the \hyperlink{def-Primed-Frame}{primed frame}s axis) described in this frames coordinates as
...
This should be expected as this shows the expected symmetry between the frames because if we were originally in the primed frame we would have the intial frame moving instead at -${v}$ in the z-direction relative to the primed frame, so we would expect that the inverse \hyperlink{def-transform}{transformation} would replace one frame's coordinates with the other and have the negative of the velocity instead.
...
For the time \hyperlink{def-transform}{transform} the $\frac{{v}{z}}{{c}^2}$ term is the cause of \hyperlink{def-simultaneity}{simultaneous} \hyperlink{def-event}{event}s in one inertial frame no longer being \hyperlink{def-simultaneity}{simultaneous} in another inertial frame, and the gamma factor ${\gamma}$, is the term that causes the overall change in how fast time flows relative in each frame.
